...
import PontoAdminModal, { type AbaPontoAdmin, type DadosTratamentosPonto, type DocumentoRepP, type EstadoAssinaturaPonto, type EventoAuditoriaPonto, type FuncionarioPonto, type PontoConfig, type PontoDiaNaoUtil, type TratamentoItemPonto } from './components/PontoAdminModal';
...
useEffect(() => {
  ...
  async function registrarTratamentoPonto(dados: { acao: 'ajuste' | 'abono'; funcionarioId: string; data: string; dataFim?: string; tipo: string; motivo: string; minutosAbonados?: number; horario?: string; tipoMarcacao?: 'entrada' | 'saida'; anexo?: File | null }): Promise<{ erro: boolean; mensagem?: string }> {
    try {
      const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token;
      if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' };
      const formulario = new FormData(); formulario.set('empresaId', empresaId); formulario.set('acao', dados.acao); formulario.set('funcionarioId', dados.funcionarioId); formulario.set('tipo', dados.tipo); formulario.set('motivo', dados.motivo);
      if (dados.acao === 'ajuste') { formulario.set('dataReferencia', dados.data); formulario.set('dadosPropostos', JSON.stringify({ horario: dados.horario, tipoMarcacao: dados.tipoMarcacao, sequencia: 1 })); }
      else { formulario.set('dataInicio', dados.data); formulario.set('dataFim', dados.dataFim || dados.data); if (dados.minutosAbonados) formulario.set('minutosAbonados', String(dados.minutosAbonados)); }
      if (dados.anexo) formulario.set('anexo', dados.anexo);
      const resposta = await fetch('/api/ponto/tratamentos', { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: formulario }); const r = await resposta.json().catch(() => null);
      return resposta.ok && !r?.erro ? { erro: false } : { erro: true, mensagem: r?.mensagem || 'Não foi possível registrar.' };
    } catch { return { erro: true, mensagem: 'Não foi possível registrar.' }; }
  }
  async function lancarBancoHorasPonto(dados: { funcionarioId: string; data: string; minutos: number; natureza: string; motivo: string }): Promise<{ erro: boolean; mensagem?: string }> { try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' }; const resposta = await fetch('/api/ponto/tratamentos', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ empresaId, acao: 'banco_lancamento', funcionarioId: dados.funcionarioId, dataReferencia: dados.data, minutos: dados.minutos, natureza: dados.natureza, motivo: dados.motivo }) }); const r = await resposta.json(); return resposta.ok && !r.erro ? { erro: false } : { erro: true, mensagem: r.mensagem }; } catch { return { erro: true, mensagem: 'Não foi possível lançar.' }; } }
  async function cadastrarBancoRegraPonto(dados: { nome: string; vigenciaInicio: string; acordoReferencia: string; prazoCompensacaoDias: number; limiteMinutos?: number }): Promise<{ erro: boolean; mensagem?: string }> { try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' }; const resposta = await fetch('/api/ponto/tratamentos', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ empresaId, acao: 'banco_regra', ...dados }) }); const r = await resposta.json(); return resposta.ok && !r.erro ? { erro: false } : { erro: true, mensagem: r.mensagem }; } catch { return { erro: true, mensagem: 'Não foi possível cadastrar a regra.' }; } }
  async function carregarTratamentosPonto(): Promise<DadosTratamentosPonto> { const vazio: DadosTratamentosPonto = { ajustes: [], abonos: [], regras: [], lancamentos: [], saldoPorFuncionario: {} }; try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return vazio; const resposta = await fetch(`/api/ponto/tratamentos?empresaId=${encodeURIComponent(empresaId)}`, { headers: { Authorization: `Bearer ${token}` } }); const r = await resposta.json(); return resposta.ok && !r.erro ? r as DadosTratamentosPonto : vazio; } catch { return vazio; } }
  async function decidirTratamentoPonto(tipo: 'ajuste' | 'abono', id: string, decisao: 'aprovado' | 'recusado' | 'cancelado', motivo: string): Promise<{ erro: boolean; mensagem?: string }> { try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' }; const resposta = await fetch('/api/ponto/tratamentos', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ empresaId, acao: 'decidir', [tipo === 'ajuste' ? 'ajusteId' : 'abonoId']: id, decisao, motivo }) }); const r = await resposta.json(); return resposta.ok && !r.erro ? { erro: false } : { erro: true, mensagem: r.mensagem }; } catch { return { erro: true, mensagem: 'Não foi possível registrar a decisão.' }; } }
  async function baixarComprovanteTratamentoPonto(tipo: 'ajuste' | 'abono', item: TratamentoItemPonto): Promise<{ erro: boolean; mensagem?: string }> { try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' }; const resposta = await fetch(`/api/ponto/tratamentos?empresaId=${encodeURIComponent(empresaId)}&anexoTipo=${tipo}&anexoId=${encodeURIComponent(item.id)}`, { headers: { Authorization: `Bearer ${token}` } }); if (!resposta.ok) { const r = await resposta.json().catch(() => null); return { erro: true, mensagem: r?.mensagem || 'Não foi possível baixar o comprovante.' }; } const arquivo = await resposta.blob(); const url = URL.createObjectURL(arquivo); const link = document.createElement('a'); link.href = url; link.download = item.comprovante_path?.split('/').at(-1) || 'comprovante'; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); return { erro: false }; } catch { return { erro: true, mensagem: 'Não foi possível baixar o comprovante.' }; } }
  async function gerarAejPonto(inicio: string, fim: string): Promise<{ erro: boolean; mensagem?: string }> { try { const { data: sessao } = await supabase.auth.getSession(); const token = sessao.session?.access_token; if (!token || !empresaId) return { erro: true, mensagem: 'Sessão não encontrada.' }; const resposta = await fetch(`/api/ponto/documentos-rep-p/aej?empresaId=${encodeURIComponent(empresaId)}&inicio=${inicio}&fim=${fim}`, { headers: { Authorization: `Bearer ${token}` } }); if (!resposta.ok) { const r = await resposta.json().catch(() => null); return { erro: true, mensagem: r?.mensagem || 'Não foi possível gerar o AEJ.' }; } const arquivo = await resposta.blob(); const url = URL.createObjectURL(arquivo); const link = document.createElement('a'); link.href = url; link.download = `AEJ-${inicio}-${fim}.zip`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); return { erro: false }; } catch { return { erro: true, mensagem: 'Não foi possível gerar o AEJ.' }; } }
...
<PontoAdminModal
  ...
  onRegistrarTratamento={registrarTratamentoPonto}
  onLancarBancoHoras={lancarBancoHorasPonto}
  onCadastrarBancoRegra={cadastrarBancoRegraPonto}
  onCarregarTratamentos={carregarTratamentosPonto}
  onDecidirTratamento={decidirTratamentoPonto}
  onBaixarComprovanteTratamento={baixarComprovanteTratamentoPonto}
  onGerarAej={gerarAejPonto}
