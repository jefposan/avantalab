import { handleCommercialNfePreparationRequest } from '@/app/vendas/lib/server/commercial-nfe-preparation-http.mjs';
...
export async function POST(request: Request, { params }: { params: Promise<{ artifactId: string }> }) {
  ...
  return handleCommercialNfePreparationRequest({ request, runtime: await getFiscalStatusRuntime(), companyId, draftId });
}
