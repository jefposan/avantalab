import { handleCommercialNfeCancellationRequest } from '@/app/vendas/lib/server/commercial-nfe-cancellation-http.mjs';
...
export async function POST(request: Request, { params }: { params: Promise<{ artifactId: string }> }) {
  ...
  return handleCommercialNfeCancellationRequest({ request, runtime: await getFiscalStatusRuntime(), companyId, emissionId });
}
