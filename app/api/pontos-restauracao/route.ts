...
async function contexto(request: Request, empresaId: string) {
  ...
  const { data: vinculo } = await admin.from('usuarios_empresa').select('user_id').eq('empresa_id', empresaId).eq('user_id', data.user.id).eq('perfil', 'gestor_master').eq('status', 'ativo').maybeSingle();
  return vinculo ? { admin, userId: data.user.id } : null;
}

export async function GET(request: Request) {
  const empresaId = new URL(request.url).searchParams.get('empresaId') || '';
  const ctx = await contexto(request, empresaId);
  if (!ctx) return NextResponse.json({ erro: true, mensagem: 'Acesso permitido apenas ao Gestor Master.' }, { status: 403 });
