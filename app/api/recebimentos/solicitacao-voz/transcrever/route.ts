...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ transcription, transcriptionMs }, {
      headers: {
        'Cache-Control': 'no-store, private',
        'Server-Timing': `auth;dur=${authMs}, hints;dur=${hintsMs}, transcribe;dur=${transcriptionMs}, total;dur=${totalMs}`,
      },
    });
  } catch (error) {
