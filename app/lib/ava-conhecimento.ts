...
- O card próximo ao centro recebe destaque e somente os vizinhos imediatos ficam levemente desfocados. O encaixe reage rapidamente depois que a rolagem termina e posiciona o card no centro vertical entre o cabeçalho principal e o menu inferior; gestos fortes continuam avançando por vários cards.
...
- No campo Produto, digite nome, código, marca ou categoria para filtrar imediatamente a lista; toque no resultado para selecionar o produto e preencher seu preço.
- Consignado não entra como venda/recebimento até ser convertido em pedido. Ao abrir um consignado, são exibidos somente produtos e quantidades; apenas a lista rola, mantendo cabeçalho, resumo e ações fixos. O botão Gerar pedido fica sempre visível enquanto o consignado estiver ativo; nele, informe com + e − quanto foi vendido de cada item, respeitando o limite disponível, e confirme: o pedido entra no histórico da cliente e as quantidades são abatidas do consignado. Na edição, os controles de quantidade mantêm o produto tocado e a posição da lista em foco. Não trate consignado como receita realizada.
