 ...
 */
// Revisado na versão 1.6.0.87: Tratamentos não repetem a jornada cadastrada no funcionário.
...
- Controle de Ponto é módulo opcional. Funcionários acessam /ponto; somente Gestor Master e Administrador vinculados à empresa configuram e administram o módulo — operadores não o enxergam. Funcionário sem dias de trabalho marcados fica em Escala variável: pode registrar ponto em qualquer dia, mas faltas, atrasos e lembretes automáticos dependem de escala fixa programada. Para encerrar o acesso, o gestor desmarca Funcionário ativo e salva: login e novas marcações são bloqueados, mas o histórico permanece disponível nos relatórios; o mesmo controle reativa o acesso. A aba Auditoria registra marcações, cadastros e mudanças de acesso e pode ser consultada por gestores. Após cada marcação, o funcionário vê um comprovante com código persistido e pode imprimi-lo. Na aba Tratamentos, o gestor seleciona o funcionário e solicita inclusão de marcação ou abono com comprovante obrigatório; a jornada é a já cadastrada no funcionário. Aprovação, recusa e cancelamento preservam a trilha. Banco de horas exige instrumento de referência, prazo de compensação e regra vigente antes de créditos ou débitos. Em Conformidade REP-P, gestores baixam AFDs, disponibilizam o manual, geram o Espelho de Ponto Eletrônico e um AEJ técnico de homologação. A substituição do aviso “em homologação” depende do registro INPI e de certificado A1 ICP-Brasil de produção válidos.
