...
- Em Preparando acesso, a Gestão mostra a etapa atual e um percentual baseado em tarefas realmente concluídas, incluindo sessão, perfis, permissões e dados financeiros. Assim que a validação de sistemas termina, a barra passa à carga de assinatura e cadastro, sem permanecer visualmente em 60%. Ao chegar a 100%, libera imediatamente a tela principal e inicia sincronizações complementares em segundo plano. Verificações que demoram além do limite são repetidas; se a preparação ficar sem progresso por tempo anormal, o aplicativo faz uma única reconexão automática e, numa nova falha, oferece Tentar novamente sem repetir recargas. O resumo comparativo dos demais perfis é atualizado logo após a entrada, sem atrasar os dados do perfil aberto.
...
SALA E NAVEGAÇÃO
- A sala de botões é a tela inicial obrigatória de cada abertura, inclusive quando os dados são restaurados pelo cache: Dashboard, Clientes, Produtos, Pedidos, Pagamentos, Agenda, Novidades, Divulgação e Informações.
...
- O card próximo ao ponto de encaixe recebe destaque e somente os vizinhos imediatos ficam levemente desfocados. O encaixe reage rapidamente depois que a rolagem termina e posiciona o card logo abaixo do cabeçalho, sem deixar o card anterior aparecer acima dele; gestos fortes continuam avançando por vários cards.
