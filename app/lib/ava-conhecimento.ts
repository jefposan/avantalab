 ...
 */
// Revisado na versão 1.6.1: o vínculo comercial do AvantaVendas foi separado
// definitivamente do destino financeiro opcional na Gestão.
...
- Para gestores habilitados, após o login a primeira tela permite escolher Gestão ou Vendas antes de carregar os dados; depois da escolha aparece Preparando acesso. Dentro do Vendas, todo usuário possui Ir para Gestão no canto direito do header fixo da sala de botões. Ao tocar, ele sempre lista os perfis ativos da Gestão vinculados à conta, ainda que exista apenas um; o usuário escolhe um perfil e confirma antes de abrir. Sem perfil na Gestão, a tela oferece criar ou ativar um perfil ou continuar somente no Vendas. O perfil criado passa a integrar a lista, mas nunca é definido automaticamente como destino financeiro do AvantaVendas. Ir para Gestão também pode ser configurado como atalho inferior. A troca preserva a sessão e, no PWA instalado, permanece na mesma janela em modo aplicativo, sem barras do navegador.
- O Vendas possui uma única conta operacional por usuário e não exige vínculo empresarial nem financeiro. O código empresarial solicita somente Novidades, Divulgação e catálogo da equipe, sempre com aprovação, sem compartilhar clientes, pedidos ou pagamentos. O destino financeiro é opcional e alterável apenas em Configurações > Integração com Gestão.
