...
- Avisos já recebidos ficam em Menu > Central de avisos e identificam o perfil financeiro de origem. Abrir ou fechar o painel não os remove; eles e o indicador permanecem até usar **Fechar aviso** ou **Fechar todas**. No iPhone, quando a Gestão confirma que não existem avisos pendentes, também remove os pushes antigos da Central de Notificações e zera o selo do ícone. O painel fica abaixo da área segura; quantidade e **Fechar todas** permanecem fixos enquanto os cards rolam. Depois do primeiro login de cada usuário no aparelho, a Gestão oferece ativar notificações; permissão já concedida é sincronizada automaticamente. **Agora não** e uma desativação manual são respeitados. Para mudar depois, use Menu > Configurações > Preferências > Receber notificações.
...
- Depois do primeiro login de cada usuário neste aparelho, o AvantaVendas oferece **Ativar notificações** ou **Agora não**. Se o iOS, Android ou navegador já tiver autorizado, o token é sincronizado sem repetir o pedido. Uma recusa ou desativação manual é respeitada; a escolha pode ser alterada depois em **Configurações > Notificações**.
