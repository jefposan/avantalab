 ...
 */
// Revisado na versão 1.6.0.79: emissão de AFD concentrada na empresa.
...
- Controle de Ponto é módulo opcional. Funcionários acessam /ponto; gestores/admins configuram e administram o módulo. Funcionário sem dias de trabalho marcados fica em Escala variável: pode registrar ponto em qualquer dia, mas faltas, atrasos e lembretes automáticos dependem de escala fixa programada. Para encerrar o acesso, o gestor desmarca Funcionário ativo e salva: login e novas marcações são bloqueados, mas o histórico permanece disponível nos relatórios; o mesmo controle reativa o acesso. A aba Auditoria registra marcações, cadastros e mudanças de acesso e pode ser consultada por gestores. Após cada marcação, o funcionário vê um comprovante com código persistido e pode imprimi-lo. Gestores acessam a aba Conformidade REP-P no painel de Ponto para baixar AFDs já gerados, gerar outro sem substituir os anteriores e disponibilizar o Manual do Sistema REP-P em PDF versionado; o /admin mantém somente certificado e registro INPI. A saída legal só é válida em produção com certificado ICP-Brasil vigente.
