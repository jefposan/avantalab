            ...
            <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setFiltrosPerfilAbertos((aberto) => !aberto)} aria-expanded={filtrosPerfilAbertos} className={`relative flex h-10 items-center justify-center gap-1.5 rounded-md border px-3 text-xs font-black uppercase ${filtrosPerfilAbertos || totalFiltrosPerfilAtivos ? 'border-cyan-300 bg-cyan-50 text-cyan-800' : 'border-slate-300 text-slate-600 hover:bg-slate-50'}`}><Icon name="filter" size={15} />Filtros{totalFiltrosPerfilAtivos > 0 && <span className="rounded-full bg-cyan-700 px-1.5 py-0.5 text-[9px] text-white">{totalFiltrosPerfilAtivos}</span>}</button>
                  <button type="button" onClick={() => { const porCriacao = perfilOrdem.startsWith('criado_em'); const ordem = porCriacao ? (perfilOrdem === 'criado_em_desc' ? 'criado_em_asc' : 'criado_em_desc') : (perfilOrdem === 'nome_asc' ? 'nome_desc' : 'nome_asc'); setPerfilOrdem(ordem); void buscarPerfis(1, { ordem }); }} aria-label={perfilOrdem.startsWith('criado_em') ? `Ordenar perfis por data de criação, ${perfilOrdem === 'criado_em_desc' ? 'mais antigos primeiro' : 'mais recentes primeiro'}` : `Ordenar perfis de ${perfilOrdem === 'nome_asc' ? 'Z a A' : 'A a Z'}`} className="flex h-10 items-center justify-center gap-1.5 rounded-md border border-slate-300 px-3 text-xs font-black uppercase text-slate-600 hover:bg-slate-50"><Icon name="filter" size={15} />Ordem {perfilOrdem === 'nome_desc' || perfilOrdem === 'criado_em_desc' ? 'Z/A' : 'A/Z'}</button>
                  ...
                  <button type="button" onClick={() => void buscarPerfis(1)} disabled={buscandoPerfis} className="flex h-10 w-[172px] shrink-0 items-center justify-center gap-2 rounded-md bg-cyan-700 px-4 text-xs font-black uppercase text-white hover:bg-cyan-800 disabled:opacity-60"><Icon name="search" size={15} />{buscandoPerfis ? 'Carregando...' : 'Carregar / Buscar'}</button>
                </div>
              </div>
