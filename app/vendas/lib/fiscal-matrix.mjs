...
function normalizeRule(rule, index) {
  ...
  return {
    ...
    recipientProfile: String(source.recipientProfile || 'Qualquer'),
...
function signature(rule) {
  return [rule.documentType, rule.operation, rule.destination, rule.recipientProfile, rule.presence, rule.issuePurpose].join('|');
}

export function validateFiscalMatrix(input, requestedDocumentScope = documentTypes) {
  const matrix = normalizeFiscalMatrix(input);
  const errors = [];
  const warnings = [];
  const documentScope = [...new Set((Array.isArray(requestedDocumentScope) ? requestedDocumentScope : documentTypes).filter((documentType) => documentTypes.includes(documentType)))];
...
export function resolveFiscalMatrixRule({ matrix: input, documentType, operation, destination, recipientProfile, presence }) {
  const matrix = normalizeFiscalMatrix(input);
  const context = {
  ...
  const specificity = (rule) => [rule.operation, rule.destination, rule.recipientProfile, rule.presence].filter((value) => value !== 'Qualquer').length;
