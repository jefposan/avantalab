...
function MoneyInput({ value, onChange, label, compact = false, disabled = false, mostrarAjustes = true, zeroComoPlaceholder = false }: { value: number; onChange: (valor: number) => void; label: string; compact?: boolean; disabled?: boolean; mostrarAjustes?: boolean; zeroComoPlaceholder?: boolean }) {
  const [texto, setTexto] = useState(formatarMoeda(value).replace('R$ ', ''));
  const [editando, setEditando] = useState(false);
  const valorFormatado = formatarMoeda(value).replace('R$ ', '');
  const zeroSemValor = zeroComoPlaceholder && value === 0;
  const exibido = editando ? texto : zeroSemValor ? '' : valorFormatado;
  return <div className={`${styles.moneyInput} ${compact ? styles.inputCompact : ''} ${!mostrarAjustes ? styles.moneyInputSimple : ''}`}><span>R$</span><input disabled={disabled} aria-label={label} inputMode="numeric" placeholder={zeroComoPlaceholder ? '0,00' : undefined} value={exibido} onFocus={(e) => { setTexto(zeroSemValor ? '' : valorFormatado); setEditando(true); if (!zeroSemValor) e.currentTarget.select(); }} onChange={(e) => { const formatado = formatarMoedaDigitada(e.target.value); setTexto(formatado); onChange(moedaDigitadaParaNumero(formatado) || 0); }} onBlur={() => setEditando(false)} />{mostrarAjustes && <i><button type="button" disabled={disabled} aria-label={`Aumentar ${label}`} onClick={() => onChange(Math.round((value + .01) * 100) / 100)}>▲</button><button type="button" disabled={disabled} aria-label={`Diminuir ${label}`} onClick={() => onChange(Math.max(0, Math.round((value - .01) * 100) / 100))}>▼</button></i>}</div>;
}
