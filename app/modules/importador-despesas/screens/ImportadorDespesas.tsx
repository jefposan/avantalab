...
function dataIsoValida(valor: string) {
  ...
  return data.getFullYear() === Number(partes[1]) && data.getMonth() === Number(partes[2]) - 1 && data.getDate() === Number(partes[3]);
}
...
function parseData(valor: unknown) {
  ...
  const normalizada = brasileira ? `${brasileira[3].length === 2 ? `20${brasileira[3]}` : brasileira[3]}-${brasileira[2].padStart(2, '0')}-${brasileira[1].padStart(2, '0')}` : texto.slice(0, 10);
...
function transformarLinhas(cabecalhos: string[], linhas: unknown[][], tiposValidos: string[] = []): DespesaImportada[] {
  ...
  return linhas.flatMap((linha) => {
    ...
    const possuiConteudo = [data, tipoDespesa, descricao, valor].filter((posicao) => posicao >= 0).some((posicao) => String(linha[posicao] ?? '').trim());
    ...
    if (!linhaDoModelo && (!Number.isFinite(bruto) || bruto === 0 || eEntrada || !eSaida)) return [];
    ...
    const pronto = Boolean(data >= 0 && parseData(linha[data]) && categoria && valorLancamento > 0);
    return [{ id: gerarId(), data: data >= 0 ? parseData(linha[data]) : '', descricaoOriginal, descricao: descricaoOriginal, valor: valorLancamento, categoria, incluir: true, confianca: pronto ? 'Alta' : 'Revisar' }];
  });
}
...
export default function ImportadorDespesas() {
  ...
  const pendencias = useMemo(() => selecionadas.filter((item) =>
    !dataIsoValida(item.data)
    || !item.categoria
    || !tiposDespesa.includes(item.categoria)
    || !Number.isFinite(item.valor)
    || item.valor <= 0
    || item.confianca === 'Revisar'
  ).length, [selecionadas, tiposDespesa]);
  ...
  async function confirmarImportacao() {
    ...
    if (selecionadas.some((despesa) => !dataIsoValida(despesa.data) || !despesa.categoria || !tiposDespesa.includes(despesa.categoria) || !Number.isFinite(despesa.valor) || despesa.valor <= 0)) { setAviso('Preencha uma data válida, escolha um tipo de despesa válido e informe um valor maior que zero em todos os lançamentos selecionados.'); return; }
    setConfirmando(true);
    try {
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
  ...
  return <main className={styles.pagina}>
    ...
    <section className={`${styles.painel} ${styles.tabelaPainel}`} aria-labelledby="titulo-lista"><div className={styles.cabecalhoSecao}><div><h2 id="titulo-lista">Despesas identificadas</h2><p>Desmarque o que não deve ser lançado e complete os itens em revisão.</p></div><span className={styles.badge}>{pendencias ? `${pendencias} pendência${pendencias === 1 ? '' : 's'}` : 'Tudo revisado'}</span></div>{!carregandoTipos && empresaId && !tiposDespesa.length && <div className={`${styles.aviso} ${styles.erro}`}>O perfil selecionado não possui tipos de despesa disponíveis. Cadastre-os na Gestão antes de confirmar.</div>}<div className={styles.tabelaRolagem}><table><thead><tr><th scope="col"><span className={styles.visualmenteOculto}>Incluir</span></th><th scope="col">Data</th><th scope="col">Descrição</th><th scope="col">Valor</th><th scope="col">Tipo da despesa</th><th scope="col">Situação</th></tr></thead><tbody>{despesasReconhecidas.map((despesa) => { const pronta = Boolean(dataIsoValida(despesa.data) && despesa.categoria && tiposDespesa.includes(despesa.categoria) && Number.isFinite(despesa.valor) && despesa.valor > 0 && despesa.confianca === 'Alta'); return <tr key={despesa.id} className={!despesa.incluir ? styles.ignorando : ''}><td><input type="checkbox" checked={despesa.incluir} disabled={confirmando || confirmado} onChange={(evento) => atualizar(despesa.id, { incluir: evento.target.checked })} aria-label={`Incluir ${despesa.descricao}`} /></td><td><input type="date" className={`${styles.campo} ${styles.data} ${!dataIsoValida(despesa.data) ? styles.erroCampo : ''}`} disabled={confirmando || confirmado} value={despesa.data} aria-invalid={!dataIsoValida(despesa.data)} onChange={(evento) => atualizar(despesa.id, { data: evento.target.value, confianca: dataIsoValida(evento.target.value) && despesa.categoria && despesa.valor > 0 ? 'Alta' : 'Revisar' })} aria-label={`Data de ${despesa.descricao}`} /></td><td><label className={styles.visualmenteOculto} htmlFor={`descricao-${despesa.id}`}>Descrição</label><input id={`descricao-${despesa.id}`} className={`${styles.campo} ${styles.descricao}`} disabled={confirmando || confirmado} value={despesa.descricao} onChange={(evento) => atualizar(despesa.id, { descricao: evento.target.value })} /><small>{despesa.descricaoOriginal}</small></td><td><label className={styles.visualmenteOculto} htmlFor={`valor-${despesa.id}`}>Valor</label><input id={`valor-${despesa.id}`} type="number" min="0.01" step="0.01" inputMode="decimal" value={despesa.valor > 0 ? despesa.valor : ''} aria-invalid={!Number.isFinite(despesa.valor) || despesa.valor <= 0} disabled={confirmando || confirmado} onChange={(evento) => { const valor = Number(evento.target.value); atualizar(despesa.id, { valor: Number.isFinite(valor) ? valor : 0, confianca: valor > 0 && dataIsoValida(despesa.data) && despesa.categoria ? 'Alta' : 'Revisar' }); }} className={`${styles.campo} ${styles.valorCampo} ${!Number.isFinite(despesa.valor) || despesa.valor <= 0 ? styles.erroCampo : ''}`} /></td><td><label className={styles.visualmenteOculto} htmlFor={`categoria-${despesa.id}`}>Tipo da despesa</label><select id={`categoria-${despesa.id}`} value={despesa.categoria} aria-invalid={!despesa.categoria || !tiposDespesa.includes(despesa.categoria)} disabled={confirmando || confirmado || carregandoTipos || !tiposDespesa.length} onChange={(evento) => atualizar(despesa.id, { categoria: evento.target.value as CategoriaDespesa, confianca: evento.target.value && dataIsoValida(despesa.data) && despesa.valor > 0 ? 'Alta' : 'Revisar' })} className={`${styles.campo} ${!despesa.categoria || !tiposDespesa.includes(despesa.categoria) ? styles.erroCampo : ''}`}><option value="">{carregandoTipos ? 'Carregando tipos…' : 'Selecionar tipo'}</option>{tiposDespesa.map((tipo) => <option key={tipo}>{tipo}</option>)}</select></td><td><span className={`${styles.situacao} ${pronta ? styles.ok : styles.revisar}`}>{pronta ? '✓ Pronto' : '! Revisar'}</span></td></tr>; })}</tbody></table></div><footer className={styles.acoes}><div><strong>{moeda(totalImportado)}</strong><span> em {selecionadas.length} despesas selecionadas</span></div><div><button className={`${styles.botao} ${styles.discreto}`} type="button" onClick={salvarRascunho} disabled={confirmando || confirmado}>Salvar rascunho</button><button className={`${styles.botao} ${styles.primario}`} type="button" onClick={() => void confirmarImportacao()} disabled={!empresaId || !selecionadas.length || pendencias > 0 || confirmando || confirmado || carregandoTipos || !tiposDespesa.length}>{confirmando ? 'Criando lançamentos…' : confirmado ? 'Despesas lançadas' : 'Confirmar e lançar despesas'}</button></div></footer></section>
