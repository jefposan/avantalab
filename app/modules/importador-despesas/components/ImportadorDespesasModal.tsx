...
function horarioSalvo(valor: string) {
  return valor.match(/\d{2}:\d{2}:\d{2}/)?.[0] || valor;
}
...
function Progresso({ percentual, titulo, detalhe, corPrimaria, darkMode }: { percentual: number; titulo: string; detalhe: string; corPrimaria: string; darkMode: boolean }) {
  return <div className="av-modal-backdrop fixed inset-0 z-[1500] flex items-center justify-center bg-slate-950/75 px-4" role="status" aria-live="polite">
    <section className={`av-modal-panel w-full max-w-md rounded-2xl p-6 shadow-2xl ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
...
function AvisoFechavel({ tipo, children, onFechar }: { tipo: 'erro' | 'atencao'; children: React.ReactNode; onFechar: () => void }) {
  ...
  const botao = tipo === 'erro'
    ? 'text-red-700 hover:bg-red-100 focus-visible:ring-red-300'
    : 'text-amber-800 hover:bg-amber-100 focus-visible:ring-amber-300';

  return <div className={`relative rounded-xl border p-3 pr-11 text-sm font-semibold transition-colors ${estilo}`} role="alert">
    {children}
    <button type="button" onClick={onFechar} className={`absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-lg font-black transition focus-visible:outline-none focus-visible:ring-2 ${botao}`} aria-label="Fechar aviso">×</button>
  </div>;
}
...
export default function ImportadorDespesasModal({ arquivo, retomarRascunho = false, empresaId, mesDestino, anoDestino, despesasCadastradas, corPrimaria, darkMode, onFechar, onConcluido, onEstadoRascunho, onArquivoDescartado, onSessaoExpirada, onSolicitarDescarte }: Props) {
  ...
  const [avisoTiposFechado, setAvisoTiposFechado] = useState(false);
  ...
  const avisoConferenciaAtivo = !confere && totalDocumento !== null;

  useEffect(() => { if (tipos.length > 0) setAvisoTiposFechado(false); }, [tipos.length]);
  useEffect(() => { if (!avisoConferenciaAtivo) setAvisoConferenciaFechado(false); }, [avisoConferenciaAtivo]);
  ...
  const botaoBase = 'inline-flex h-11 items-center justify-center rounded-xl px-3 text-xs font-black transition-[transform,border-color,box-shadow] duration-150 ease-out active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100 motion-reduce:transform-none motion-reduce:transition-none';
  const botaoCabecalhoNeutro = `${botaoBase} border border-white/40 bg-white/10 text-white shadow-sm hover:border-white focus-visible:ring-white/90`;
  const estiloBotaoSecundario = {
    backgroundColor: `color-mix(in srgb, ${corPrimaria} ${darkMode ? '22%' : '8%'}, transparent)`,
    borderColor: `color-mix(in srgb, ${corPrimaria} ${darkMode ? '62%' : '30%'}, transparent)`,
    color: darkMode ? '#ffffff' : corPrimaria,
  };
  ...
  return <div className="av-modal-backdrop fixed inset-0 z-[1500] flex items-center justify-center bg-slate-950/75 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="titulo-importador">
    <section className={`av-modal-panel flex w-full max-w-6xl flex-col overflow-hidden rounded-2xl shadow-2xl ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
