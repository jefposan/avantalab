...
import PorCategoria from '@/app/components/PorCategoria';
import Relatorio from '@/app/components/Relatorio';
import ModalTermos from '@/app/components/ModalTermos';
import ModalPrivacidade from '@/app/components/ModalPrivacidade';
...
import type { StatusCadastroPerfil } from '@/app/lib/cadastro-perfil';
import { PAISES } from '@/app/lib/paises';
import {
  formatarMoeda,
  formatarDescricao,
  corEhClara,
...
const [validandoTelefoneObrigatorio, setValidandoTelefoneObrigatorio] = useState(false);
  ...
  const empresaOrigemCriacao = resolverEmpresaOrigemDaCriacao(
    empresaId,
  ...
  useEffect(() => {
    ...
    void (async () => {
      setQuotaPerfis(null);
      setQuotaPerfisCarregando(true);
      setQuotaPerfisErro('');
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      if (!token) {
        if (ativo) {
          setQuotaPerfisErro('Sua sessão expirou. Entre novamente para verificar o plano.');
          setQuotaPerfisCarregando(false);
        }
        return;
      }
      ...
      if (!ativo) return;
      if (resposta?.ok && dados?.ativo) {
        setQuotaPerfis(dados);
      } else {
        setQuotaPerfisErro(
          String(dados?.mensagem || 'Não foi possível verificar o plano e as vagas disponíveis.'),
        );
      }
      setQuotaPerfisCarregando(false);
    })();
...
const renderizarOpcoesInicioPerfilEmpresa = () => {
  if (quotaPerfisCarregando) {
    return (
      <p role="status" aria-live="polite" className="text-xs font-bold leading-snug text-sky-900">
        Verificando seu plano e as vagas disponíveis...
      </p>
    );
  }

  if (quotaPerfis?.compartilhaAcesso) {
        ...
        {(['trial', 'assinar'] as const).map((modo) => {
          ...
          return (
            <button
              key={modo}
              type="button"
              onClick={() => setInicioEmpresaModo(modo)}
              className={`rounded-lg px-2 py-2 text-[11px] font-black uppercase tracking-wide transition ${ativo ? 'bg-sky-700 text-white shadow' : 'bg-white text-slate-600 hover:bg-sky-100'}`}
            >
  ...
  async function instalarModulo(moduloId: string) {
    ...
    try {
      ...
      if (acessoComercial === 'business') {
        ...
        const resposta = await fetch('/api/cobranca/modulos/assinar', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ empresaId, moduloId }),
        });
        const json = await resposta.json();
        if (!resposta.ok) throw new Error(json.mensagem || 'Não foi possível iniciar a assinatura do módulo.');
        if (json.invoiceUrl) window.open(json.invoiceUrl, '_blank', 'noopener,noreferrer');
        abrirAviso(
          'Assinatura do módulo iniciada',
          json.invoiceUrl
            ? 'Abrimos a cobrança mensal de R$ 14,90 em outra aba. O módulo será liberado assim que o pagamento for confirmado.'
            : 'A cobrança está sendo preparada. O módulo será liberado após a confirmação do pagamento.',
          undefined,
          'sucesso',
        );
        return;
...
<div className="mt-6 text-left">
    ...
    {(['empresa', 'pessoal'] as TipoPerfil[]).map((tipo) => {
      ...
      return (
        ...
        >
          ...
          {tipo === 'pessoal' && criandoPerfilAdicional && criarPerfilPessoalBloqueado() && <span className="ml-1 text-xs" title="Recurso Premium">🔒</span>}
        </button>
      );
    })}
  </div>
  ...
  {COBRANCA_ATIVA && tipoPerfilInicialNormalizado === 'empresa' && (
    <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-3">
      {renderizarOpcoesInicioPerfilEmpresa()}
    </div>
  )}
</div>
...
<div className="mt-6 grid grid-cols-1 sm:grid-cols-[1fr_140px] gap-3">
  <button
    ...
    disabled={criandoEmpresaInicial || quotaPerfisCarregando}
  ...
  >
    {quotaPerfisCarregando ? 'Verificando plano...' : criandoEmpresaInicial ? 'Criando ambiente...' : 'Concluir'}
  </button>
    ...
    return (
      <>
        <PaywallEmpresa
          ...
          onEscolherPlano={faturaPendenteUrl ? undefined : (plano, ciclo) => {
            setPlanoCadastroPaywall(plano);
            setCicloCadastroPaywall(ciclo);
          }}
        ...
        {cicloCadastroPaywall && empresaId && (
          <CadastroPerfilModal
            ...
            onCancelar={() => { setCicloCadastroPaywall(null); setPlanoCadastroPaywall(null); }}
            onConcluido={async (status, cobranca) => {
              setCadastroPerfilStatus(status);
              const resultado = await iniciarAssinatura(planoCadastroPaywall || (tipoPerfilAtual === 'pessoal' ? 'pessoal_premium' : 'business'), cicloCadastroPaywall, cobranca, true);
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              ...
              <button
                ...
                style={estiloTemaPrimario}
              >
                {quotaPerfisCarregando ? 'Verificando...' : criandoEmpresaInicial ? 'Criando...' : 'Criar perfil'}
              </button>
            </div>
          </div>
        )}
