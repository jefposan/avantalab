...
export default function Graficos({ meses, lancamentos, faturamentos, corPrimaria, darkMode, formatarMoeda }: GraficosProps) {
  ...
  const dadosAnuais = useMemo(() => {
    ...
    const dados = meses.map(mes => {
      const desp = lancamentos.filter(l => l.mes === mes).reduce((acc, l) => acc + l.valor, 0);
      const fat = faturamentos[mes] || 0;
      if (desp > maxValor) maxValor = desp;
      if (fat > maxValor) maxValor = fat;
      return { mes, desp, fat };
    });
    return { dados, maxValor: maxValor > 0 ? maxValor : 1 }; // Evita divisão por zero
      ...
      Comparativo Anual
          ...
          <div className="flex-1 flex items-end gap-2 h-72 pb-4 border-b border-slate-200/20 relative mt-4">
            ...
            {dadosAnuais.dados.map((item) => {
              ...
              return (
                <div key={item.mes} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                  ...
                  <div className={`absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 text-white text-[10px] p-2 rounded shadow-xl pointer-events-none z-10 whitespace-nowrap flex flex-col items-center`}>
                    <span className="text-[#00b050] font-bold">{formatarMoeda(item.fat)}</span>
                    <span className="text-red-400 font-bold">{formatarMoeda(item.desp)}</span>
                  </div>
                  ...
                  <div className="flex items-end justify-center w-full gap-1 h-full z-0">
                    ...
                    <div 
                      ...
                      style={{ height: `${alturaFat}%`, minHeight: item.fat > 0 ? '4px' : '0' }}
                    ></div>
                    {/* Barra Despesa */}
                    <div 
                      className="w-1/3 bg-red-600 rounded-t-sm shadow-sm transition-all duration-500 hover:brightness-110" 
                      style={{ height: `${alturaDesp}%`, minHeight: item.desp > 0 ? '4px' : '0' }}
                    ></div>
                  </div>
                  <span className={`text-[10px] font-bold mt-3 uppercase tracking-wider ${textMuted} truncate w-full text-center`}>
