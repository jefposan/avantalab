...
export default function PublicSeoPage({ data }: { data: PublicSeoPageData }) {
  ...
  return <main className={styles.page}>
    ...
    <header className={styles.header}><nav className={styles.nav} aria-label="Navegação principal"><Link className={styles.brand} href="/" aria-label="AvantaLab — página inicial"><Image src="/images/landing/logo-avantalab.png" alt="AvantaLab" width={154} height={40} priority /></Link><div className={styles.navActions}><AcessoPublicoLink className={styles.login} modo="entrar">Entrar</AcessoPublicoLink><AcessoPublicoLink className={styles.button} modo="cadastro">Começar grátis</AcessoPublicoLink></div></nav></header>
    <section className={styles.hero}><div className={styles.wrap}><p className={styles.eyebrow}>{data.eyebrow}</p><h1>{data.title}</h1><p className={styles.lead}>{data.intro}</p><div className={styles.heroActions}><AcessoPublicoLink className={styles.button} modo="cadastro">Começar grátis</AcessoPublicoLink><Link className={styles.secondary} href="/">Conhecer o AvantaLab</Link></div></div></section>
    ...
    <section className={styles.cta}><div className={styles.wrap}><h2>{data.cta}</h2><p>Comece pelo Free para uso pessoal ou conheça os planos empresariais para acompanhar uma operação em crescimento.</p><AcessoPublicoLink className={styles.button} modo="cadastro">Criar conta grátis</AcessoPublicoLink></div></section>
