...
export default function ProcessandoImagemModal({ aberto, darkMode, titulo = 'Processando imagem' }: ProcessandoImagemModalProps) {
  ...
  return (
    <div className="av-modal-backdrop fixed inset-0 z-[1400] flex items-center justify-center bg-slate-950/75 px-4" role="status" aria-live="polite">
      <section className={`av-modal-panel w-full max-w-xs overflow-hidden rounded-2xl shadow-2xl ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
