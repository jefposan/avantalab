...
const textoNumero = (valor: string) => Number(String(valor).replace(',', '.'));
...
export default function CatalogoProdutosVendas({ empresaId, darkMode, corPrimaria }: Props) {
  ...
  const editar = (produto: Produto) => setFormulario({ ...vazio, ...produto, preco_custo: String(produto.preco_custo), preco_venda: String(produto.preco_venda), imagem_url: produto.imagem_url || '', sku: produto.sku || '', descricao: produto.descricao || '', marca: produto.marca || '', categoria: produto.categoria || '', ncm: produto.ncm || '', codigo_barras: produto.codigo_barras || '' });
  const salvar = async () => {
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto p-4"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-base font-black">Pacote de produtos</h3><p className={`mt-1 text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Os campos abaixo alimentam o Vendas Mobile. Dados fiscais podem ser completados sem afetar o catálogo atual.</p></div><span className="rounded-full bg-cyan-500/10 px-3 py-1 text-[10px] font-black uppercase text-cyan-700">{produtos.length} produtos</span></div><div className="grid gap-4 xl:grid-cols-[minmax(0,.85fr)_minmax(0,1.15fr)]"><section className={`self-start rounded-xl border p-4 ${painel}`}><h4 className="text-sm font-black">{formulario.id ? 'Editar produto' : 'Novo produto'}</h4><p className="mt-1 text-[10px] font-bold text-cyan-600">Obrigatórios: nome, custo e preço de venda.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{[['nome','Nome'],['marca','Marca'],['categoria','Categoria'],['sku','SKU'],['unidade','Unidade'],['preco_custo','Preço de custo'],['preco_venda','Preço de venda'],['codigo_barras','EAN / GTIN'],['ncm','NCM']].map(([chave, rotulo]) => <label key={chave} className="text-[9px] font-black uppercase opacity-70">{rotulo}<input value={String(formulario[chave] || '')} onChange={(e) => mudar(chave, e.target.value)} inputMode={chave.startsWith('preco') ? 'decimal' : undefined} className={`mt-1 h-9 w-full rounded-lg border px-2 text-xs font-bold normal-case ${campo}`} /></label>)}</div><label className="mt-2 block text-[9px] font-black uppercase opacity-70">Descrição<textarea value={String(formulario.descricao || '')} onChange={(e) => mudar('descricao', e.target.value)} rows={3} className={`mt-1 w-full rounded-lg border p-2 text-xs normal-case ${campo}`} /></label><label className="mt-2 block text-[9px] font-black uppercase opacity-70">Link da imagem<input value={String(formulario.imagem_url || '')} onChange={(e) => mudar('imagem_url', e.target.value)} className={`mt-1 h-9 w-full rounded-lg border px-2 text-xs normal-case ${campo}`} /></label><input ref={arquivoRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => void enviarImagem(e.target.files?.[0])} /><button type="button" onClick={() => arquivoRef.current?.click()} disabled={salvando} className="mt-2 h-9 w-full rounded-lg border border-cyan-300 text-[10px] font-black uppercase text-cyan-700">Enviar imagem</button><label className="mt-2 flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={formulario.ativo !== false} onChange={(e) => mudar('ativo', e.target.checked)} /> Produto ativo</label><div className="mt-3 flex gap-2"><button type="button" onClick={() => void salvar()} disabled={salvando} className="h-10 flex-1 rounded-lg text-xs font-black uppercase text-white disabled:opacity-60" style={{ backgroundColor: corPrimaria }}>{salvando ? 'Salvando...' : 'Salvar produto'}</button>{formulario.id && <button type="button" onClick={() => setFormulario(vazio)} className="h-10 rounded-lg border px-3 text-xs font-black">Cancelar</button>}</div></section><section><h4 className="text-sm font-black">Produtos do pacote</h4><div className="mt-2 overflow-x-auto rounded-xl border"><table className="min-w-full text-left text-xs"><thead className={darkMode ? 'bg-slate-800' : 'bg-slate-50'}><tr><th className="px-3 py-2">Produto</th><th className="px-3 py-2">SKU</th><th className="px-3 py-2">Custo</th><th className="px-3 py-2">Venda</th><th /></tr></thead><tbody>{carregando ? <tr><td colSpan={5} className="px-3 py-10 text-center">Carregando...</td></tr> : produtos.length ? produtos.map((produto) => <tr key={produto.id} className={`border-t ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}><td className="px-3 py-2"><b className="block">{produto.nome}</b><small className="text-slate-500">{produto.marca || 'Sem marca'} · {produto.categoria || 'Sem categoria'}</small></td><td className="px-3 py-2">{produto.sku || '—'}</td><td className="px-3 py-2">R$ {Number(produto.preco_custo).toFixed(2)}</td><td className="px-3 py-2">R$ {Number(produto.preco_venda).toFixed(2)}</td><td className="px-3 py-2"><button type="button" onClick={() => editar(produto)} className="rounded-md border px-2 py-1 text-[10px] font-black text-cyan-700">Editar</button></td></tr>) : <tr><td colSpan={5} className="px-3 py-10 text-center text-slate-500">Nenhum produto cadastrado.</td></tr>}</tbody></table></div></section></div>{erro && <p className="mt-3 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-xs font-bold text-red-700">{erro}</p>}</div>;
}
