...
export default function CatalogoProdutosVendas({ empresaId, darkMode, corPrimaria }: Props) {
  ...
  const salvar = async () => {
    ...
    if (!nome || !Number.isFinite(custo) || custo < 0 || !Number.isFinite(venda) || venda <= 0) {
      setErro('Nome, preço de custo e preço de venda são obrigatórios.');
      return;
    }
    setSalvando(true);
    setErro('');
