...
export default function PontoFacialLiveness({ identityPoolId }: { identityPoolId: string }) {
  ...
  useEffect(() => {
    ...
    const organizarAcoes = () => {
      ...
      if (!iniciar || !cancelarBotao || !iniciar.parentElement) return;
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-[radial-gradient(circle_at_top,#075985_0%,#003e73_42%,#020617_100%)] px-3 py-5 sm:p-8" role="dialog" aria-modal="true" aria-label="Verificação facial">
    ...
    <AvantaCard title={sessao ? 'Verificação em andamento' : emCadastro ? 'Cadastro facial' : 'Confirmação facial'} corPrimaria="#007f99" hideDragHandle hideMenu className="avanta-facial mx-auto max-w-xl text-slate-900" bodyClassName="!min-h-[calc(100dvh-130px)] !p-0 overflow-hidden" style={temaFacial} plato={<span className="rounded-full bg-cyan-100 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-cyan-800">Ponto seguro</span>}>
      ...
      <div className={sessao ? 'flex min-h-[calc(100dvh-195px)] flex-col p-3' : 'p-4 sm:p-5'}>
        ...
        {sessao && <><div ref={detectorRef} className="relative z-10 min-h-0 flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white pt-2 shadow-sm"><FaceLivenessDetector sessionId={sessao.sessaoId} region={sessao.regiao} displayText={textosPtBr} components={{ PhotosensitiveWarning: AvisoFotossensibilidadeOculto }} onAnalysisComplete={concluir} onError={() => setMensagem('A câmera não conseguiu concluir a verificação. Mantenha o celular na vertical e tente novamente.')} onUserCancel={cancelar} /></div><div className="flex justify-center pt-1"><button ref={cancelarRef} type="button" className="avanta-facial-cancelar h-10 min-h-10 w-[220px] rounded-[10px] border border-slate-300 bg-white px-4 text-sm font-black text-slate-600 shadow-sm transition active:scale-[0.98] hover:bg-slate-50 focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#1687D9]" onClick={cancelar}>Cancelar verificação</button></div></>}
