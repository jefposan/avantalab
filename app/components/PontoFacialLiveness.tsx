...
export default function PontoFacialLiveness({ identityPoolId }: { identityPoolId: string }) {
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-[radial-gradient(circle_at_top,#075985_0%,#003e73_42%,#020617_100%)] px-3 py-5 sm:p-8" role="dialog" aria-modal="true" aria-label="Verificação facial">
    <style>{`.avanta-facial .amplify-button--primary{min-height:56px!important;width:220px!important;border-radius:12px!important;background:#1687D9!important;border-color:#1687D9!important;color:#fff!important;font-size:15px!important;font-weight:800!important;box-shadow:0 8px 18px rgba(22,135,217,.24)!important}.avanta-facial .amplify-button--primary:active{transform:scale(.98)}.avanta-facial .amplify-button--primary:focus-visible{outline:3px solid rgba(22,135,217,.35)!important;outline-offset:3px}.avanta-facial .amplify-liveness-start-screen{padding-bottom:8px!important}`}</style>
    <AvantaCard title={emCadastro ? 'Cadastro facial' : 'Confirmação facial'} corPrimaria="#007f99" hideDragHandle hideMenu className="avanta-facial mx-auto max-w-xl text-slate-900" bodyClassName="!min-h-[calc(100dvh-130px)] !p-0 overflow-hidden" style={temaFacial} plato={<span className="rounded-full bg-cyan-100 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-cyan-800">Ponto seguro</span>}>
      ...
      <div className="p-4 sm:p-5">
        <div className="rounded-xl border border-cyan-100 bg-cyan-50 px-3 py-2 text-xs font-semibold leading-snug text-slate-700" role="status">
          {mensagem || 'Posicione o rosto inteiro no contorno, com boa luz. Siga as orientações na tela e fique imóvel quando solicitado.'}
        </div>
        {sessao && <div className="relative z-10 mt-3 min-h-[calc(100dvh-320px)] overflow-hidden rounded-2xl border border-slate-200 bg-white pt-1 shadow-sm"><FaceLivenessDetector sessionId={sessao.sessaoId} region={sessao.regiao} displayText={textosPtBr} components={{ PhotosensitiveWarning: AvisoFotossensibilidadeOculto }} onAnalysisComplete={concluir} onError={() => setMensagem('A câmera não conseguiu concluir a verificação. Mantenha o celular na vertical e tente novamente.')} onUserCancel={cancelar} /><div className="flex justify-center px-4 pb-3 pt-2"><button type="button" className="min-h-14 w-[220px] rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-600 shadow-sm transition active:scale-[0.98] hover:bg-slate-50 focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#1687D9]" onClick={cancelar}>Cancelar verificação</button></div></div>}
