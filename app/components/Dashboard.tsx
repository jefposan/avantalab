...
interface DashboardProps {
  ...
  onExcluirAporteCaixinha: (movimento: { id: string; lancamentoId: string | null; descricao: string; valor: number }) => void;
...
export default function Dashboard({
  ...
  const [ocultarValoresCentrosCusto, setOcultarValoresCentrosCusto] = useState(iniciarValoresOcultos);
  ...
  const caixinhaMensagemSucesso = /adicionado|adicionada|atualizado|salvo|excluído/i.test(caixinhaMensagem);
  useEffect(() => {
    setOcultarValores(iniciarValoresOcultos);
    setOcultarValoresPerfis(iniciarValoresOcultos);
    setOcultarValoresCentrosCusto(iniciarValoresOcultos);
    ...
    caixinha: (
      <div className={`${bgCard} card-radius-avantalab w-full overflow-hidden rounded-2xl border-2 shadow-lg transition-colors`} style={{ borderColor: corPrimaria }}>
        ...
        <div className="space-y-4 p-5">
          ...
          {caixinhaMensagem && (
            <p role="status" className={`text-center text-xs font-bold ${caixinhaMensagemSucesso ? 'text-emerald-500' : 'text-red-500'}`}>
              {caixinhaMensagem}
            </p>
          )}
      ...
      <div className={`${bgCard} card-radius-avantalab w-full overflow-hidden rounded-2xl border-2 shadow-lg transition-colors`} style={{ borderColor: corPrimaria }}>
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 px-6 py-3 text-sm font-bold uppercase tracking-wider" style={{ backgroundColor: corPrimaria, color: textoSobreCorPrimaria }}>
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              type="button"
              onClick={() => setOcultarValoresCentrosCusto(!ocultarValoresCentrosCusto)}
              className="flex h-6 w-6 items-center justify-center rounded text-white/75 transition hover:bg-white/15 hover:text-white"
              title={ocultarValoresCentrosCusto ? 'Mostrar valores' : 'Ocultar valores'}
              aria-label={ocultarValoresCentrosCusto ? 'Mostrar valores do card Centros de custo' : 'Ocultar valores do card Centros de custo'}
            >
              {ocultarValoresCentrosCusto ? (
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                </svg>
              ) : (
