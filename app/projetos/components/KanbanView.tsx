    ...
    return <section key={status} className={styles.kanbanColumn}
      ...
      <div>{items.map((node) => {
        ...
        return <article key={node.id} draggable={!readOnly} onDragStart={(event) => { if (readOnly) return; event.dataTransfer.setData('text/avanta-node', node.id); event.dataTransfer.effectAllowed = 'move'; }}>
          <button type="button" onClick={() => onOpen(node.id)}><span className={styles.kanbanType}>{node.icon} {node.type === 'marco' ? 'Marco' : 'Tarefa'}</span><strong>{node.title}</strong>{node.description && <p>{node.description}</p>}<div className={styles.tagList}>{node.tags.slice(0, 2).map((tag) => <span key={tag}>{tag}</span>)}</div><footer><div className={styles.avatarStack}>{assigned.map((person) => <span key={person!.id} style={{ background: person!.color }}>{person!.initials}</span>)}</div><span>{node.dueDate ? new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: 'short' }).format(new Date(`${node.dueDate}T12:00:00`)) : 'Sem prazo'}</span></footer></button>
        </article>;
