...
export function MapCanvas({
  ...
  useEffect(() => {
    const move = (event: PointerEvent) => {
      ...
      if (drag && event.pointerId === drag.pointerId) setDragPosition({ nodeId: drag.nodeId, x: drag.origin.x + (event.clientX - drag.x) / viewport.scale, y: drag.origin.y + (event.clientY - drag.y) / viewport.scale });
    };
  ...
  return <div ref={containerRef} className={`${styles.canvas} ${pan ? styles.canvasPanning : ''}`} tabIndex={0} aria-label="Mapa visual do projeto" onWheel={(event) => { event.preventDefault(); zoomAt(viewport.scale * (event.deltaY > 0 ? 0.9 : 1.1), event.clientX, event.clientY); }} onPointerDown={(event) => {
    ...
    {selectedNode && !readOnly && <div className={styles.nodeActionBar} role="toolbar" aria-label={`Ações de ${selectedNode.title}`}>
      <Tooltip texto="Adicionar filho (Tab)" posicao="bottom"><button type="button" onClick={() => onCreateChild(selectedNode.id)}><Icon name="child" /><span>Filho</span></button></Tooltip>
      <Tooltip texto="Adicionar irmão (Shift+Tab)" posicao="bottom"><button type="button" onClick={() => onCreateSibling(selectedNode.id)}><Icon name="sibling" /><span>Irmão</span></button></Tooltip>
      <Tooltip texto="Conexão hierárquica" posicao="bottom"><button type="button" onClick={() => startConnect('hierarquica')}><Icon name="link" /><span>Filho</span></button></Tooltip>
      <Tooltip texto="Conexão livre" posicao="bottom"><button type="button" onClick={() => startConnect('livre')}><Icon name="link" /><span>Relacionar</span></button></Tooltip>
      <div className={styles.colorPalette}><div className={styles.presetColors}><header><span>Cores pré-definidas</span><button type="button" onClick={() => setEditingPresets(!editingPresets)}>{editingPresets ? 'Concluir' : 'Ajustar'}</button></header><div>{Array.from({ length: 5 }, (_, index) => colorPresets[index] ?? '').map((color, index) => editingPresets ? <Tooltip key={index} texto={`Definir cor ${index + 1}`} posicao="bottom"><label className={styles.presetColorEditor}><input type="color" value={color || '#5b7c91'} onChange={(event) => { const next = Array.from({ length: 5 }, (_, item) => colorPresets[item] ?? ''); next[index] = event.target.value; onColorPresetsChange(next); }} /><i style={{ background: color || undefined }} /></label></Tooltip> : <button key={index} type="button" className={styles.presetColor} style={{ background: color || undefined }} disabled={!color} onClick={() => onUpdateColor(selectedNode.id, color)} aria-label={color ? `Aplicar cor pré-definida ${index + 1}` : `Cor pré-definida ${index + 1} não definida`} />)}</div></div><Tooltip texto="Cor livre" posicao="bottom"><label className={styles.colorAction}><span className={styles.srOnly}>Cor livre</span><input type="color" value={selectedNode.color} onChange={(event) => onUpdateColor(selectedNode.id, event.target.value)} /></label></Tooltip></div>
      <Tooltip texto="Duplicar" posicao="bottom"><button type="button" onClick={() => onDuplicate(selectedNode.id)}><Icon name="copy" /><span>Duplicar</span></button></Tooltip>
      <Tooltip texto="Abrir detalhes" posicao="bottom"><button type="button" onClick={() => onOpenDetails(selectedNode.id)}><Icon name="chevron" /><span>Detalhes</span></button></Tooltip>
      {project.nodes.some((node) => node.parentId === selectedNode.id) && <Tooltip texto={selectedNode.collapsed ? 'Expandir ramificação' : 'Recolher ramificação'} posicao="bottom"><button type="button" onClick={() => onToggleCollapse(selectedNode.id)}><span>{selectedNode.collapsed ? '⊞' : '⊟'}</span><span>{selectedNode.collapsed ? 'Expandir' : 'Recolher'}</span></button></Tooltip>}
      <Tooltip texto="Excluir" posicao="bottom"><button type="button" className={styles.destructiveAction} onClick={() => onDelete(selectedNode.id)}><Icon name="trash" /><span>Excluir</span></button></Tooltip>
    </div>}
