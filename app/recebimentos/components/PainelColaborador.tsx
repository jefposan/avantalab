...
type Props = {
  ...
  onRegistrar: (subempresaId: string, valorRecebido: number, observacao: string) => void;
  // Baixa individual de uma parcela em atraso já existente.
  onReceberCobranca: (recebimentoId: string, valorRecebido: number, observacao: string) => void;
};
...
export default function PainelColaborador({ colaborador, empresas, subempresas, recebimentos, onRegistrar, onReceberCobranca }: Props) {
  ...
  function handleConfirmar(subempresaId: string, valor: number, obs: string, resumo: ResumoRecebimento) {
    onRegistrar(subempresaId, valor, obs);
    setFormAberto(false);
    setComprovante(resumo);
  }

  function handleReceberCobranca(recebimentoId: string, valor: number, obs: string, resumo: ResumoRecebimento) {
    onReceberCobranca(recebimentoId, valor, obs);
    setFormAberto(false);
    setComprovante(resumo);
  }
