...
type Props = {
  ...
  onConfirmar: (subempresaId: string, valorRecebido: number, observacao: string, resumo: ResumoRecebimento) => void;
  // Baixa de uma cobrança em atraso específica (uma a uma).
  onReceberCobranca: (recebimentoId: string, valorRecebido: number, observacao: string, resumo: ResumoRecebimento) => void;
  onCancelar: () => void;
};
...
export default function FormularioRecebimento({ empresas, subempresas, recebimentos, onConfirmar, onReceberCobranca, onCancelar }: Props) {
  ...
  function confirmar() {
    ...
    if (cobranca) {
      onReceberCobranca(cobranca.id, Number(valorRecebido.toFixed(2)), observacao, resumo);
    } else if (sub) {
      onConfirmar(sub.id, Number(valorRecebido.toFixed(2)), observacao, resumo);
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button type="button" className={`${styles.btn} ${styles.btnPrimary}`} onClick={confirmar} style={{ flex: 2 }}>
