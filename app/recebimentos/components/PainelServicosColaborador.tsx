...
type Props = { colaborador: Colaborador; empresas: Empresa[]; subempresas: Subempresa[]; servicos: Servico[]; podeRegistrar: boolean; podeAgendar: boolean; registroInicial?: PreparacaoRegistroServicoVoz | null; onRegistroInicialConsumido?: () => void; onRegistrar: (empresaId: string, subempresaId: string | null, clienteNome: string, assinatura: string, avaliacao: AvaliacaoServico, observacao: string, servicoId?: string) => Promise<void> | void; onAgendar: (empresaId: string, subempresaId: string | null, data: string, tipo: Exclude<TipoServico, 'rotina'>) => Promise<void> | void; };

export default function PainelServicosColaborador({ colaborador, empresas, subempresas, servicos, podeRegistrar, podeAgendar, registroInicial, onRegistroInicialConsumido, onRegistrar, onAgendar }: Props) {
  ...
  return <div className={styles.mobileWrap}>
    ...
    {acaoAtiva === 'registrar' && formAberto && <div id="servico-registro" className={styles.servicoCardPwa}><AvantaCard title="Registrar serviços" hideDragHandle hideMenu style={avantaShell.cardStyle} bodyStyle={avantaShell.bodyStyle}><FormularioServico empresas={empresas} subempresas={subempresas} servicos={servicos} selecaoInicial={registroPreparado} onCancelar={() => { setRegistroPreparado(null); setFormAberto(false); setAcaoAtiva(null); }} onConfirmar={async (...dados) => { await onRegistrar(...dados); setRegistroPreparado(null); setFormAberto(false); setAcaoAtiva(null); setSucesso(true); }} /></AvantaCard></div>}
    {sucesso && <div className={styles.sucessoServicoOverlay} role="presentation"><div className={styles.sucessoServico} role="dialog" aria-modal="true" aria-labelledby="operacao-concluida-titulo"><AvantaCard className={styles.sucessoServicoCard} title={<span id="operacao-concluida-titulo">Operação concluída</span>} titleClassName={styles.sucessoServicoTitulo} hideDragHandle hideMenu style={{ ...avantaShell.cardStyle, ['--plato-w' as string]: '0%', ['--avanta-title-left' as string]: '30px' }} bodyStyle={avantaShell.bodyStyle}><p>O serviço foi atualizado e já está disponível para a gestão.</p><button ref={continuarRef} type="button" className={`${styles.btn} ${styles.btnPrimary}`} onClick={() => setSucesso(false)}>Continuar</button></AvantaCard></div></div>}
