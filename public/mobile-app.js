(function () {
  ...
  async function concluirLoginSocialNativoMobile(usuario) {
    ...
    // A sessão já foi confirmada. Mantém uma única cena de preparação enquanto
    // perfis e dados obrigatórios são carregados, sem voltar ao login.
    state.pronto = false;
    render();
    ...
    try {
      ...
      if (perfisCarregados === false) {
        exibirFalhaDeAcessoMobile('Sua sessão foi iniciada, mas os perfis demoraram para responder. Tente novamente.');
        return;
      }
      ...
      state.loginAcao = '';
      exibirFalhaDeAcessoMobile(mensagemErro(erro, 'Não foi possível concluir o login social.'));
    }
  }
  ...
  var state = {
    ...
    entradaValor: '',
    menuAberto: false,
    menuOrganizacaoAberto: false,
    menuOrganizacaoAnimacao: '',
    menuSistemasAberto: false,
    menuSistemasAnimacao: '',
  ...
  async function abrirGerenciamentoModuloVendasMobile() {
    if (premiumPessoalBloqueadoMobile()) {
    ...
    try {
      ...
      if (state.vendasMobileModuloAtivo) {
        mostrarToast('O módulo Vendas Mobile já está ativo neste perfil.');
        return;
      }
  ...
  async function ativarVendasMobileNoPerfil() {
    ...
    try {
      ...
      state.ativacaoVendasMobileErro = '';
      render();
      mostrarToast('Módulo Vendas Mobile ativado neste perfil.');
    } catch (error) {
  ...
  function avaliarSistemaInicialMobile() {
    // A rota /mobile abre sempre a Gestão. Não há troca automática entre os
    // aplicativos: cada produto é aberto explicitamente pelo usuário.
    return false;
  }

  async function prepararSistemaInicialAntesDosDadosMobile() {
    state.vendasMobileModuloAtivo = false;
    // /mobile sempre abre a Gestão. A disponibilidade do Vendas vem da mesma
    // consulta de módulos dos dados do perfil; reparações opcionais ocorrem
    // depois da abertura e não atrasam o acesso principal.
  ...
  function abrirAgendaMobile() {
    ...
    state.visao = 'agenda';
    state.menuAberto = false;
    state.menuOrganizacaoAberto = false;
    state.menuOrganizacaoAnimacao = '';
    state.menuSistemasAberto = false;
  ...
  function configurarRealtimeAprovacoesVendasMobile() {
    try {
      var empresaId = state.empresa && state.empresa.id;
      if (!empresaId || !podeGerenciarAprovacoesVendasMobile()) {
        if (window._avaRealtimeAprovacoesVendas) db.removeChannel(window._avaRealtimeAprovacoesVendas);
        if (window._avaIntervaloAprovacoesVendas) window.clearInterval(window._avaIntervaloAprovacoesVendas);
        window._avaRealtimeAprovacoesVendas = null;
        window._avaRealtimeAprovacoesVendasEmpresaId = '';
        window._avaIntervaloAprovacoesVendas = null;
        limparAprovacoesVendasMobile();
        return;
      }
      if (window._avaRealtimeAprovacoesVendasEmpresaId === empresaId) return;
      if (window._avaRealtimeAprovacoesVendas) db.removeChannel(window._avaRealtimeAprovacoesVendas);
      if (window._avaIntervaloAprovacoesVendas) window.clearInterval(window._avaIntervaloAprovacoesVendas);

      window._avaRealtimeAprovacoesVendasEmpresaId = empresaId;
      window._avaRealtimeAprovacoesVendas = db
        .channel('vendas_mobile_aprovacoes_mobile_' + empresaId)
        .on('postgres_changes',
      ...
      window._avaIntervaloAprovacoesVendas = window.setInterval(function () {
        carregarAprovacoesVendasMobile(false);
      }, 30000);
    } catch (e) {}
  }
  ...
  var temporizadorRenderSegundoPlanoMobile = null;

  function agendarRenderSegundoPlanoMobile(empresaIdEsperada) {
    if (temporizadorRenderSegundoPlanoMobile) {
      window.clearTimeout(temporizadorRenderSegundoPlanoMobile);
    }
    temporizadorRenderSegundoPlanoMobile = window.setTimeout(function () {
      temporizadorRenderSegundoPlanoMobile = null;
      if (!state.autenticado || !state.pronto || !state.empresa) return;
      if (empresaIdEsperada && state.empresa.id !== empresaIdEsperada) return;
      render();
    }, 280);
  }

  async function carregarResumoPontoMobile(renderizar) {
    ...
    var funcionariosAtivos = (respostas[0].data || []).filter(function (funcionario) {
      return Boolean(funcionario.user_id);
    });
    var diasNaoUteis = respostas[2].error ? [] : (respostas[2].data || []);
    if (diaNaoUtilPontoMobile(periodo.data, diasNaoUteis)) {
      state.pontoFuncionariosHoje = 0;
      state.pontoResumo = [];
      state.pontoFuncionarios = funcionariosAtivos.map(function (funcionario) {
        return { userId: funcionario.user_id, nome: funcionario.nome, status: 'sem_jornada' };
      });
      state.pontoResumoCarregando = false;
      if (renderizar !== false) render();
      return;
    ...
    var funcionariosHoje = funcionariosAtivos.filter(function (funcionario) {
    ...
    var resumoPorUsuario = {};
    resumo.forEach(function (item) { resumoPorUsuario[item.userId] = item.status; });
    var funcionariosHojePorUsuario = {};
    funcionariosHoje.forEach(function (funcionario) { funcionariosHojePorUsuario[funcionario.user_id] = true; });
    state.pontoFuncionarios = funcionariosAtivos.map(function (funcionario) {
      return {
        userId: funcionario.user_id,
        nome: funcionario.nome,
        status: resumoPorUsuario[funcionario.user_id] || (funcionariosHojePorUsuario[funcionario.user_id] ? 'em_dia' : 'sem_jornada'),
      };
    });
    state.pontoResumoCarregando = false;
    if (renderizar !== false) render();
  ...
  async function sincronizarAgendaSupabase(renderizar) {
    if (!state.usuario || !state.usuario.id) return;
    try {
  ...
  function podeGerenciarAprovacoesVendasMobile() {
    return podeGerenciarUsuarios();
  }

  function formatarDataHoraAprovacaoMobile(valor) {
  ...
  function limparAprovacoesVendasMobile() {
    ...
    state.aprovacoesVendasAba = 'pendentes';
  }

  async function carregarAprovacoesVendasMobile(renderizar) {
    if (!state.empresa || !state.empresa.id || !podeGerenciarAprovacoesVendasMobile()) {
      limparAprovacoesVendasMobile();
      if (renderizar !== false) render();
      return;
    }

    state.aprovacoesVendasCarregando = true;
    state.aprovacoesVendasErro = '';
    if (renderizar !== false) render();

    try {
      var respostas = await Promise.all([
        db.from('vendas_mobile_solicitacoes_acesso')
      ...
      state.aprovacoesVendasPendentes = solicitacoes.filter(function (item) {
        ...
        return {
          ...
          nome: String(item.nome || 'Solicitante'),
          email: String(item.email || 'E-mail não informado'),
      ...
      state.aprovacoesVendasCarregando = false;
      if (renderizar !== false || state.menuAberto || state.modalMenu === 'aprovacoesVendas') render();
    }
  }

  async function abrirAprovacoesVendasMobile() {
    if (!podeGerenciarAprovacoesVendasMobile()) {
      await mostrarAvisoSistemaMobile('Acesso não permitido', 'Somente Gestor Master ou Administrador pode analisar aprovações.');
      return;
    }
    state.aprovacoesVendasAba = 'pendentes';
    abrirModalMenu('aprovacoesVendas');
    await carregarAprovacoesVendasMobile();
  }

  async function analisarSolicitacaoVendasMobile(solicitacao, aprovar) {
    if (!solicitacao || state.aprovacoesVendasProcessandoId) return;
    var escolha = await solicitarDialogoSistemaMobile({
      titulo: aprovar ? 'Aprovar acesso?' : 'Rejeitar solicitação?',
      rotulo: 'Vendas Mobile',
      mensagem: aprovar
        ? solicitacao.nome + ' terá acesso ao Vendas Mobile neste perfil.'
        : solicitacao.nome + ' não terá acesso ao Vendas Mobile neste perfil.',
      variante: aprovar ? 'alerta' : 'destrutiva',
      acoes: [
        { valor: 'cancelar', rotulo: 'Cancelar', estilo: 'secundaria' },
        { valor: 'confirmar', rotulo: aprovar ? 'Aprovar acesso' : 'Rejeitar', estilo: aprovar ? 'primaria' : 'perigosa' },
      ],
    });
    if (escolha !== 'confirmar') return;

    state.aprovacoesVendasProcessandoId = solicitacao.id;
    state.aprovacoesVendasErro = '';
    render();
    try {
      var resposta = await db.rpc('analisar_solicitacao_vendas_mobile_rpc', {
  ...
  async function carregarEmpresas(usuarioId, usuarioAutenticado) {
    ...
    if (acessoComTelefone) {
      var vinculosParaSincronizarTelefone = vinculos.data.filter(function (vinculo) {
        return vinculo.telefone !== acessoComTelefone.telefone || vinculo.telefone_confirmado !== true;
      });
      if (vinculosParaSincronizarTelefone.length) {
        await db
          .from('usuarios_empresa')
          .update({
          ...
          .in('id', vinculosParaSincronizarTelefone.map(function (vinculo) { return vinculo.id; }));
      }
  ...
  async function garantirFixasDoMes(empresaId, anoCarregado) {
    try {
      ...
      if (Number(anoCarregado) !== anoAtual) return 0;
  ...
  async function carregarDados() {
    ...
    state.carregando = true;
    // Se a cena de preparação já está visível, apenas seu progresso é
    // atualizado no DOM. Evita reconstruir a mesma tela no meio da transição.
    if (!telaPreparacaoAcessoMobileVisivel()) render();

    // Assinatura, cadastro e dados financeiros são independentes.
    ...
    window.setTimeout(function () {
      carregarResumoPerfisMobile().then(function () {
        ...
        ) agendarRenderSegundoPlanoMobile(empresaIdResumo);
      }).catch(function (erro) {
    ...
    window.setTimeout(function () {
      ...
      executarSegundoPlano('Falha ao atualizar despesas fixas', function () {
        return garantirFixasDoMes(empresaIdResumo, Number(anoResumo)).then(function (quantidadeCriada) {
          if (
            quantidadeCriada > 0 &&
            state.empresa && state.empresa.id === empresaIdResumo &&
      ...
      executarSegundoPlano('Falha ao sincronizar agenda em segundo plano', function () {
        return sincronizarAgendaSupabase(false).then(function () {
          agendarRenderSegundoPlanoMobile(empresaIdResumo);
        });
      });
      ...
      executarSegundoPlano('Falha ao iniciar aprovações do Vendas em tempo real', configurarRealtimeAprovacoesVendasMobile);
      executarSegundoPlano('Falha ao iniciar ponto em tempo real', configurarRealtimePontoMobile);
      executarSegundoPlano('Falha ao carregar resumo do ponto', function () {
        return carregarResumoPontoMobile(false).then(function () {
          agendarRenderSegundoPlanoMobile(empresaIdResumo);
        });
      });
      executarSegundoPlano('Falha ao carregar notificações', function () {
        return carregarNotificacoesNaoLidas(false).then(function () {
          agendarRenderSegundoPlanoMobile(empresaIdResumo);
        });
      });
      executarSegundoPlano('Falha ao preparar integração com o Vendas', function () {
        if (!state.vendasMobileModuloAtivo || !podeGerenciarUsuarios()) return;
        return db.rpc('garantir_acessos_gestor_vendas_mobile_rpc').then(function (resultado) {
          if (resultado.error) {
            console.warn('Não foi possível reparar o acesso integrado ao Vendas Mobile:', resultado.error);
          }
        });
      });
  ...
  async function salvarEdicaoLancamentoSelecionado(confirmarPrevista) {
    ...
    var anoDestino = confirmarAgora ? periodoConfirmacao.ano : Number(state.ano);
    var mesDestino = confirmarAgora ? periodoConfirmacao.mes : mesItem;
    var limite = maxDias(mesDestino, anoDestino);

    // Lê TODOS os campos ANTES do render() — o render reconstrói o DOM e zera os inputs.
    var diaInformado = Number(campo('editar-dia'));
    var dia = confirmarAgora ? periodoConfirmacao.dia : diaInformado;
    ...
    if (tipo === 'receita') {
      ...
      if (!eraPrevista || confirmarAgora) {
        var totalAtual = state.faturamentos[mesDestino] || 0;
        if (confirmarAgora && !state.centrosCustoAtivo) {
          ...
          totalAtual = Number((faturamentoAtual.data && faturamentoAtual.data.valor) || 0);
        }
      ...
      var ehFuturaEditada = dataFutura(anoDestino, indiceMes(mesDestino), dia);
    ...
    state.modalAcao = null;
    await carregarDados();
    concluirAplicacaoLancamentoMobile(confirmarAgora ? (tipo === 'receita' ? 'Receita confirmada com a data de hoje.' : 'Despesa confirmada com a data de hoje.') : (tipo === 'receita' ? 'Receita atualizada.' : 'Despesa atualizada.'));
  }
  ...
  function cenaAcessoGestaoMobile(card, classeExtra) {
    return (
      '<section class="gestao-login-screen' + (classeExtra ? ' ' + classeExtra : '') + '">' +
  ...
  function rolarPaginaParaCampoCadastroNoNavegador(campo) {
    if (isStandalone() || !campo || document.activeElement !== campo) return;
    try { campo.scrollIntoView({ behavior: 'auto', block: 'center' }); } catch (erro) {}
  }

  function agendarFocoCadastroNoNavegador(campo) {
    if (isStandalone()) return;
    window.requestAnimationFrame(function () {
      rolarPaginaParaCampoCadastroNoNavegador(campo);
    });
    window.setTimeout(function () {
      rolarPaginaParaCampoCadastroNoNavegador(campo);
    }, 180);
  ...
  function telaLogin() {
    ...
    if (state.modoCadastro) {
      var cadastroNoNavegador = !isStandalone();
      return cenaAcessoGestaoMobile(
        '<form data-cadastro-mobile-card class="mx-auto min-w-0 shrink-0 ' + (cadastroNoNavegador ? '' : 'overflow-y-auto ') + 'rounded-3xl border border-white/35 p-3 text-slate-900 shadow-2xl backdrop-blur-xl" onsubmit="return false" style="grid-row:2;justify-self:center;width:min(88%,320px);background-color:rgba(255,255,255,.18);' + (cadastroNoNavegador ? '' : 'max-height:calc(100dvh - env(safe-area-inset-top) - env(safe-area-inset-bottom) - 108px);overscroll-behavior:contain;') + '">' +
          '<div class="mb-2"><h1 class="text-xl font-black text-slate-900">Criar cadastro</h1></div>' +
          telaCadastro() +
        '</form>',
        cadastroNoNavegador ? 'gestao-login-screen-cadastro-navegador' : ''
      );
    }
    var maxH = boasVindas
      ? 'none'
  ...
  function acoesHeaderMobileHtml() {
    ...
    return avisosHtml
      ? '<div class="flex shrink-0 items-center gap-2">' + avisosHtml + '</div>'
  ...
  function telaApp() {
    ...
    return (
      '<div class="mobile-app-shell fixed inset-0 flex min-w-0 flex-col overflow-hidden ' + (state.darkMode ? 'mobile-dark bg-slate-950 text-slate-100' : 'mobile-light bg-slate-100 text-slate-900') + '" style="overscroll-behavior:none;">' +
        ...
        '<div class="mx-auto grid w-full min-w-0 max-w-md grid-cols-1 gap-3 px-3 pt-11 sm:px-4">' +
  ...
  function itemNavegacaoInferiorHtml(id, tipo, rotulo, ativo) {
    ...
    var premiumInativo = premiumPessoalBloqueadoMobile() && tipo === 'agenda';
  ...
  function pontoRelatorioMobileHtml() {
    if (!state.pontoRelatorioUsuarioId) {
      var funcionarios = state.pontoFuncionarios || [];
      return '<div class="grid gap-2">' +
        '<p class="rounded-xl px-3 py-2 text-xs font-semibold ' + (state.darkMode ? 'bg-cyan-950/40 text-cyan-100' : 'bg-cyan-50 text-cyan-900') + '">Funcionarios ativos do Controle de Ponto. Toque em um nome para ver os registros de hoje.</p>' +
        (funcionarios.length ? funcionarios.map(function (item) {
          var visual = {
            atraso: ['Atraso', 'bg-amber-100 text-amber-700'],
            falta: ['Falta', 'bg-red-100 text-red-700'],
            incompleto: ['Incompleto', 'bg-violet-100 text-violet-700'],
            em_dia: ['Em dia', 'bg-emerald-100 text-emerald-700'],
            sem_jornada: ['Sem jornada hoje', 'bg-slate-200 text-slate-600'],
          }[item.status] || ['Sem ocorrencia', 'bg-slate-200 text-slate-600'];
          return '<button type="button" data-ponto-relatorio-user="' + escapeHtml(item.userId) + '" data-ponto-relatorio-nome="' + escapeHtml(item.nome) + '" class="flex min-h-11 items-center justify-between gap-2 rounded-xl border px-3 text-left ' + (state.darkMode ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-slate-50') + '"><span class="min-w-0 flex-1 truncate text-xs font-bold ' + (state.darkMode ? 'text-slate-100' : 'text-slate-800') + '">' + escapeHtml(item.nome) + '</span><span class="shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase ' + visual[1] + '">' + visual[0] + '</span></button>';
        }).join('') : '<p class="py-5 text-center text-sm font-semibold text-slate-500">Nenhum funcionario ativo.</p>') +
      '</div>';
    }
    ...
    if (acao.tipo === 'receita') {
      return (
        ...
        (item.status === 'prevista'
          ? '<p class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-[11px] font-semibold leading-relaxed text-emerald-800">Ao confirmar, esta receita será registrada automaticamente na data de hoje.</p><div class="grid grid-cols-2 gap-2"><button id="salvar-edicao-lancamento" type="button" ' + (state.carregando ? 'disabled ' : '') + 'class="h-11 rounded-xl border border-slate-300 bg-white px-2 text-[11px] font-black uppercase tracking-wide text-slate-700 disabled:opacity-60">Salvar previsto</button><button id="confirmar-edicao-prevista" type="button" ' + (state.carregando ? 'disabled ' : '') + 'class="h-11 rounded-xl bg-emerald-600 px-2 text-[11px] font-black uppercase tracking-wide text-white disabled:opacity-60">Confirmar hoje</button></div><button id="excluir-lancamento-edicao" type="button" ' + (state.carregando ? 'disabled ' : '') + 'class="h-11 rounded-xl border border-red-200 bg-red-50 px-2 text-xs font-black uppercase tracking-wide text-red-700 disabled:opacity-60">Excluir</button>'
          : '<div class="grid grid-cols-2 gap-2"><button id="salvar-edicao-lancamento" type="button" ' + (state.carregando ? 'disabled ' : '') + 'class="h-11 rounded-xl bg-cyan-500 px-2 text-xs font-black uppercase tracking-wide text-slate-950 disabled:opacity-60">' + (state.carregando ? 'Salvando...' : 'Salvar') + '</button><button id="excluir-lancamento-edicao" type="button" ' + (state.carregando ? 'disabled ' : '') + 'class="h-11 rounded-xl border border-red-200 bg-red-50 px-2 text-xs font-black uppercase tracking-wide text-red-700 disabled:opacity-60">Excluir</button></div>') +
        '</div>'
      );
    }
  ...
  function menuLateralHtml() {
    ...
    var bordaDivisorConfig = dk ? 'border-slate-700' : 'border-slate-200';
    var organizacaoAberto = !!state.menuOrganizacaoAberto;
    var sistemasAberto = !!state.menuSistemasAberto;
    var configAberto = !!state.menuConfigAberto;
    var conteudoVendasDisponivel = !!(state.vendasMobileModuloAtivo && podeGerenciarConteudoVendas());
    function animacaoSubgrupoMenu(valor) {
      return valor === 'sair'
    ...
    var configSubItens = configAberto ? (
      '<div id="menu-config-conteudo" class="cfg-sub-group mx-0.5 mt-1 grid gap-1 rounded-xl border p-2 ' + overflowSubgrupoMenu(state.menuConfigAnimacao) + ' ' + (dk ? 'border-slate-700 bg-slate-800/60' : 'border-slate-200') + '" style="' + configAnimacao + (dk ? '' : 'background:#FCFFFF;box-shadow:inset 0 1px 0 rgba(255,255,255,.8);') + '">' +
        ...
        (podeGerenciarAprovacoesVendasMobile()
          ...
          : '') +
        '<button id="menu-gerenciar" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)] active:scale-[0.99]" style="order:9;' + (dk ? '' : 'background:linear-gradient(90deg,#EAF6FF 0%,#FFFFFF 78%);border-color:#C8E4F6;') + '">' +
          '<div class="flex items-center gap-2">' +
        ...
        '<button id="menu-inicio-valores-ocultos" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)]" style="order:3;' + (dk ? '' : 'background:linear-gradient(90deg,#F0F9FF 0%,#FFFFFF 78%);border-color:#C9E9F7;') + '">' +
          ...
          '</div>' +
        '</button>' +
        '<button id="menu-notificacoes" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)] active:scale-[0.99]' + (premiumInativo ? ' grayscale opacity-60' : '') + '" style="order:5;' + (dk ? '' : 'background:linear-gradient(90deg,#E8F9FD 0%,#FFFFFF 78%);border-color:#C4EAF4;') + '"' + (premiumInativo ? ' aria-disabled="true"' : '') + '>' +
          '<div class="flex items-center gap-2">' +
            ...
            '<span class="min-w-0 flex-1"><span class="block text-[11px] font-black">Receber notificações neste aparelho</span><span class="mt-0.5 block truncate text-[9px] font-semibold text-slate-500">' + (state.notificacoesAtivas ? 'Ativas' : 'Inativas') + '</span></span>' +
        ...
        '<button id="menu-usuario" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)] active:scale-[0.99]' + (premiumInativo ? ' grayscale opacity-60' : '') + '" style="order:10;' + (dk ? '' : 'background:linear-gradient(90deg,#E8FAF7 0%,#FFFFFF 78%);border-color:#BFE5E0;') + '"' + (premiumInativo ? ' aria-disabled="true"' : '') + '>' +
          ...
          '</div>' +
        '</button>' +
        '<p class="mt-1 border-t ' + bordaDivisorConfig + ' px-2 pb-0.5 pt-2 text-[9px] font-black uppercase tracking-[0.18em] text-slate-500" style="order:11">Dados e segurança</p>' +
        '<button id="menu-backup" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)] active:scale-[0.99]' + (premiumInativo ? ' grayscale opacity-60' : '') + '" style="order:12;' + (dk ? '' : 'background:linear-gradient(90deg,#EAF3FF 0%,#FFFFFF 78%);border-color:#C8DCF5;') + '"' + (premiumInativo ? ' aria-disabled="true"' : '') + '>' +
          '<div class="flex items-center gap-2">' +
          ...
          '</div>' +
        '</button>' +
        '<button id="menu-restauracao" type="button" class="min-h-11 rounded-xl border ' + bordaBase + ' px-2.5 py-2 text-left shadow-[0_4px_11px_rgba(15,23,42,.05)] active:scale-[0.99]' + (premiumInativo ? ' grayscale opacity-60' : '') + '" style="order:13;' + (dk ? '' : 'background:linear-gradient(90deg,#EEEEFF 0%,#FFFFFF 78%);border-color:#D2D2F1;') + '"' + (premiumInativo ? ' aria-disabled="true"' : '') + '>' +
          '<div class="flex items-center gap-2">' +
            ...
            '<span class="min-w-0 flex-1"><span class="block text-[11px] font-black">Restaurar backup</span><span class="mt-0.5 block truncate text-[9px] font-semibold text-slate-500">Importar um backup do AvantaLab</span></span>' +
          '</div>' +
        '</button>' +
        ...
        '<aside id="menu-aside" class="flex h-full w-[84vw] max-w-[348px] flex-col overflow-hidden rounded-r-3xl ' + (dk ? 'bg-slate-950 text-slate-100' : 'text-slate-900') + ' p-3 shadow-2xl" style="background:' + (dk ? '#020617' : 'linear-gradient(180deg,#F8FBFF 0%,#F4F8FC 100%)') + ';backface-visibility:hidden;-webkit-backface-visibility:hidden;' + limiteTopoMenuIos + animacaoPainel + '">' +
          ...
          '<div class="grid gap-1.5">' +
            ...
            '<button id="menu-organizacao-toggle" type="button" aria-expanded="' + (organizacaoAberto ? 'true' : 'false') + '" aria-controls="menu-organizacao-conteudo" class="mt-1 min-h-11 rounded-xl border border-sky-200 px-2.5 py-2 text-left shadow-[0_5px_13px_rgba(15,23,42,.07)] transition active:scale-[0.99]" style="background:' + (dk ? '#0F172A' : 'linear-gradient(90deg,#E8F3FF 0%,#FAFCFF 100%)') + '">' +
              '<div class="flex items-center gap-2">' +
                '<span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-sky-600 text-white shadow-sm">' + iconeMenuLateralSvg('menu-organizar-dashboard') + '</span>' +
                '<span class="min-w-0 flex-1"><span class="block text-xs font-black leading-none">Organizar tela inicial</span><span class="mt-1 block truncate text-[10px] font-semibold leading-none text-slate-500">Cards e atalhos inferiores</span></span>' +
                '<span class="ml-auto flex h-6 w-6 shrink-0 items-center justify-center text-slate-500">' + chevronExpansaoMenuSvg(organizacaoAberto) + '</span>' +
              '</div>' +
            '</button>' +
            organizacaoSubItens +
            '<button id="menu-sistemas-toggle" type="button" aria-expanded="' + (sistemasAberto ? 'true' : 'false') + '" aria-controls="menu-sistemas-conteudo" class="mt-1 min-h-11 rounded-xl border border-cyan-200 px-2.5 py-2 text-left shadow-[0_5px_13px_rgba(15,23,42,.07)] transition active:scale-[0.99]" style="background:' + (dk ? '#0F172A' : 'linear-gradient(90deg,#E1F7FC 0%,#F8FDFF 100%)') + '">' +
              '<div class="flex items-center gap-2">' +
                '<span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-cyan-700 text-white shadow-sm">' + iconeMenuLateralSvg('menu-sistemas') + '</span>' +
                '<span class="min-w-0 flex-1"><span class="block text-xs font-black leading-none">Sistemas</span><span class="mt-1 block truncate text-[10px] font-semibold leading-none text-slate-500">Vendas Mobile e conteúdos</span></span>' +
                '<span class="ml-auto flex h-6 w-6 shrink-0 items-center justify-center text-slate-500">' + chevronExpansaoMenuSvg(sistemasAberto) + '</span>' +
              '</div>' +
            '</button>' +
            sistemasSubItens +
            '<button id="menu-config-toggle" type="button" aria-expanded="' + (configAberto ? 'true' : 'false') + '" aria-controls="menu-config-conteudo" class="mobile-config-main-btn mt-1 min-h-11 rounded-xl border border-slate-300 px-2.5 py-2 text-left text-slate-800 shadow-[0_5px_13px_rgba(15,23,42,.09)] transition active:scale-[0.99]" style="background:' + (configAberto ? 'linear-gradient(90deg,#B8C3D0 0%,#A5B2C1 100%)' : 'linear-gradient(90deg,#CBD5E1 0%,#B4C0CE 100%)') + '">' +
              '<div class="flex items-center gap-2">' +
                ...
                '<span class="min-w-0 flex-1"><span class="mobile-config-main-title block text-xs font-black leading-none text-slate-800">Configurações</span><span class="mobile-config-main-subtitle mt-1 block truncate text-[10px] font-semibold leading-none text-slate-600">Preferências, conta e segurança</span></span>' +
                '<span class="ml-auto flex h-6 w-6 shrink-0 items-center justify-center text-slate-600">' + chevronExpansaoMenuSvg(configAberto) + '</span>' +
              '</div>' +
            '</button>' +
            configSubItens +
          '</div>' +
  ...
  function chevronExpansaoMenuSvg(aberto) {
    var caminho = aberto ? 'm5 15 7-7 7 7' : 'm5 9 7 7 7-7';
    return '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" aria-hidden="true"><path d="' + caminho + '" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  }
  ...
  function iconeMenuLateralSvg(id) {
    ...
    var caminhos = {
      ...
      'menu-aprovacoes-vendas': '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm7 1 2 2 4-4"/>',
      'menu-sistemas': '<rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/>',
  ...
  function menuBotaoHtml(id, titulo, subtitulo, desativado, visualInativo) {
    var estilos = {
      ...
      'menu-sistemas': ['linear-gradient(90deg,#E0F2FE 0%,#F8FDFF 100%)', '#BAE6FD', 'linear-gradient(135deg,#0284C7,#003E73)', '#FFFFFF'],
  ...
  function modalMenuHtml() {
    var titulo = {
      ...
      aprovacoesVendas: 'Aprovações do Vendas',
    ...
    var ajudaCategoriasCabecalho = state.modalMenu === 'categorias'
      ? '<button id="menu-ajuda-categorias" type="button" class="flex h-9 w-9 items-center justify-center rounded-full border border-white/25 bg-white/10 text-white transition active:scale-95" aria-label="Abrir instruções sobre categorias" title="Instruções sobre categorias"><span class="flex h-6 w-6 items-center justify-center rounded-full border border-white/70 text-xs font-black">i</span></button>'
      : '';
    return (
      '<div id="modal-menu-overlay" class="fixed inset-0 ' + camadaModal + ' flex items-center justify-center overflow-hidden bg-slate-950/90 px-3 pt-4" style="' + estiloOverlay + '">' +
        '<section class="mx-auto flex w-full max-w-md flex-col overflow-hidden rounded-3xl ' + (state.darkMode ? 'bg-slate-900 text-slate-100' : 'bg-white text-slate-900') + ' shadow-2xl" style="' + estiloPainel + '">' +
          '<div class="shrink-0 flex items-center justify-between gap-3 px-4 py-3 text-white" style="background-color:#003E73">' +
            ...
            '<div class="flex items-center gap-1.5">' + ajudaCategoriasCabecalho + '<button id="fechar-modal-menu" type="button" class="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white" aria-label="Fechar">' + iconeFecharGeometricoMobile() + '</button></div>' +
          '</div>' +
  ...
  function conteudoModalMenuHtml() {
    ...
    if (state.modalMenu === 'aprovacoesVendas') return aprovacoesVendasMobileHtml();
    ...
    return (
      '<div class="grid gap-2">' +
        state.empresas.map(function (empresa) {
          ...
          // O gradiente e a borda ficam explícitos no próprio botão. Em alguns
          // WebViews, a combinação de gradiente utilitário e borda transparente
          // deixava uma faixa escura no recorte arredondado à direita.
          var estiloDisponivel = !carregando && !perfilAtual
            ? ' style="background:linear-gradient(135deg,#003E73 0%,#1687D9 100%);border-color:#0C78B8;box-shadow:none;background-clip:padding-box"'
            : '';
          var classeVisual = carregando
            ? 'border-blue-600 bg-cyan-50 text-cyan-800 shadow-sm'
            : (perfilAtual
              ? 'cursor-not-allowed border-slate-200 bg-slate-100 text-slate-500'
              : 'text-white hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1687D9] focus-visible:ring-offset-2');
    ...
    if (state.aprovacoesVendasCarregando) {
      ...
      conteudo = '<div class="grid gap-3">' + pendentes.map(function (solicitacao) {
        ...
        return '<article class="rounded-2xl border p-4 ' + superficie + '">' +
          '<div class="flex items-start justify-between gap-3"><div class="min-w-0"><h3 class="truncate text-sm font-black">' + escapeHtml(solicitacao.nome) + '</h3><p class="mt-0.5 truncate text-xs font-semibold ' + textoSuave + '">' + escapeHtml(solicitacao.email) + '</p></div><span class="shrink-0 rounded-full bg-amber-400/15 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-amber-700">Pendente</span></div>' +
          '<div class="mt-3 rounded-xl border p-2.5 text-xs ' + interno + '"><span class="block text-[9px] font-black uppercase tracking-wide ' + textoSuave + '">Perfil</span><strong class="mt-0.5 block truncate">' + escapeHtml(nomeEmpresa(state.empresa)) + '</strong></div>' +
          '<div class="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-semibold ' + textoSuave + '">' +
            (solicitacao.telefone ? '<span>' + escapeHtml(solicitacao.telefone) + '</span>' : '') +
            '<span>Solicitado em ' + escapeHtml(formatarDataHoraAprovacaoMobile(solicitacao.solicitadoEm)) + '</span>' +
          '</div>' +
          '<div class="mt-4 grid grid-cols-2 gap-2"><button type="button" data-rejeitar-solicitacao-vendas="' + escapeHtml(solicitacao.id) + '"' + (processando ? ' disabled' : '') + ' class="min-h-11 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-black text-red-600 disabled:opacity-60">Rejeitar</button><button type="button" data-aprovar-solicitacao-vendas="' + escapeHtml(solicitacao.id) + '"' + (processando ? ' disabled' : '') + ' class="min-h-11 rounded-xl bg-[#003E73] px-3 py-2 text-xs font-black text-white disabled:opacity-60">' + (processando ? 'Salvando…' : 'Aprovar') + '</button></div>' +
        '</article>';
      }).join('') + '</div>';
    } else {
  ...
  function render(forcarDuranteEdicao) {
    ...
    bind('menu-gerenciar-modulo-vendas', function () { fecharMenuLateralAnimado(abrirGerenciamentoModuloVendasMobile); });
    ...
    Array.prototype.forEach.call(document.querySelectorAll('[data-aprovar-solicitacao-vendas]'), function (botao) {
      botao.addEventListener('click', function () {
        var id = botao.getAttribute('data-aprovar-solicitacao-vendas');
        var solicitacao = (state.aprovacoesVendasPendentes || []).find(function (item) { return item.id === id; });
        analisarSolicitacaoVendasMobile(solicitacao, true);
      });
    });
    Array.prototype.forEach.call(document.querySelectorAll('[data-rejeitar-solicitacao-vendas]'), function (botao) {
      botao.addEventListener('click', function () {
    ...
    bind('menu-pontos-restauracao', function () { acionarBackupMobile('pontos'); });
    function alternarGrupoMenuMobile(grupo) {
      var grupos = {
        organizacao: { aberto: 'menuOrganizacaoAberto', animacao: 'menuOrganizacaoAnimacao', conteudo: 'menu-organizacao-conteudo', botao: 'menu-organizacao-toggle' },
        sistemas: { aberto: 'menuSistemasAberto', animacao: 'menuSistemasAnimacao', conteudo: 'menu-sistemas-conteudo', botao: 'menu-sistemas-toggle' },
        configuracoes: { aberto: 'menuConfigAberto', animacao: 'menuConfigAnimacao', conteudo: 'menu-config-conteudo', botao: 'menu-config-toggle' },
      };
      var atual = grupos[grupo];
      if (!atual) return;
      if (state[atual.aberto]) {
        if (state[atual.animacao] === 'sair') return;
        state[atual.animacao] = 'sair';
        render();
        setTimeout(function () {
          state[atual.aberto] = false;
          state[atual.animacao] = '';
          render();
          window.requestAnimationFrame(function () {
            var botaoFechado = document.getElementById(atual.botao);
            if (botaoFechado) {
              try { botaoFechado.focus({ preventScroll: true }); }
              catch (e) { botaoFechado.focus(); }
            }
          });
        }, 225);
        return;
      ...
      Object.keys(grupos).forEach(function (chave) {
        state[grupos[chave].aberto] = false;
        state[grupos[chave].animacao] = '';
      });
      state[atual.aberto] = true;
      state[atual.animacao] = 'entrar';
      render();
      window.requestAnimationFrame(function () {
        var botaoAberto = document.getElementById(atual.botao);
        if (botaoAberto) {
          try { botaoAberto.focus({ preventScroll: true }); }
          catch (e) { botaoAberto.focus(); }
        }
      });
      setTimeout(function () {
        if (state[atual.aberto] && state[atual.animacao] === 'entrar') state[atual.animacao] = '';
      }, 270);
      setTimeout(function () {
        var conteudo = document.getElementById(atual.conteudo);
        if (conteudo) conteudo.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      }, 285);
    }
  ...
  async function iniciar() {
    ...
    // de rolagem. O PWA preserva o comportamento nativo já validado.
    document.addEventListener('focusin', function (e) {
      ...
      agendarFocoCadastroNoNavegador(campoCadastro);
    });

    if (window.visualViewport && !window._avaCadastroNavegadorPaginaBound) {
      window._avaCadastroNavegadorPaginaBound = true;
      var acompanharCampoCadastroNoViewport = function () {
        if (isStandalone()) return;
        ...
        agendarFocoCadastroNoNavegador(campoAtivo);
      };
      window.visualViewport.addEventListener('resize', acompanharCampoCadastroNoViewport);
      window.visualViewport.addEventListener('scroll', acompanharCampoCadastroNoViewport);
    }
