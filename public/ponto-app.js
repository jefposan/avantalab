(function () {
  ...
  async function registroServiceWorkerPonto() {
    ...
    try { return await navigator.serviceWorker.register('/ponto-sw.js?v=10', { scope: '/ponto' }); }
    catch (e) { return null; }
  }
    ...
    return (
      '<div class="fixed inset-0 flex flex-col items-center justify-center overflow-y-auto bg-slate-100 px-5 py-6">' +
        '<div class="w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl">' +
          ...
          '<button id="ponto-comprovante-imprimir" type="button" class="mt-3 h-11 w-full rounded-xl border border-slate-300 bg-white text-xs font-black uppercase tracking-wide text-slate-700">Imprimir versão simples</button>' +
  ...
  function telaPontoPremium() {
    ...
    var acaoCentral = proxima
      ...
      : '<div class="ponto-action is-complete" role="status"><span class="ponto-action-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="m5 12 4 4L19 6"/></svg></span><strong class="ponto-action-complete-title">Jornada<br>Concluída</strong><small>Registros completos</small></div>';
