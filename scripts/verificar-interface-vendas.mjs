...
exigir(
  comprovantePagamento.includes('const MARGEM_INFERIOR_RODAPE = 78;')
    ...
    && comprovantePagamento.includes("const COR_BORDA_CARD = '#BFDED1';")
    && comprovantePagamento.includes("cabecalho: '#078F63'")
    && comprovantePagamento.includes("valor: '#24C978'")
    && comprovantePagamento.includes("saldo: '#0B7A58'")
    && comprovantePagamento.includes('avantalab-payment-receipt-bg.webp')
    && comprovantePagamento.includes('retangulo(ctx, 44, 42, 992, 260, 36, PALETA_PAGAMENTO.cabecalho)')
...
exigir(
  cliente.includes('async function carregarDivulgacao()')
    ...
    && cliente.includes('loadAll, carregarDivulgacao, carregarConteudosSecundarios, loadClientFinancial')
    && aplicacao.includes('if (entradaDivulgacao) {')
    && aplicacao.includes("else void atualizarDivulgacao('entrada');")
    && aplicacao.includes('void carregarConteudosSecundariosVendas(true);')
