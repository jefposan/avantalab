-- Pagamentos representam recebimentos já realizados. Datas futuras não podem
-- compor o resultado financeiro enviado ao Gestão.
create or replace function public.validar_data_pagamento_vendas_mobile()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.data_pagamento > (now() at time zone 'America/Sao_Paulo')::date then
    raise exception 'Pagamento não pode ter data futura.';
  end if;
  return new;
end;
$$;

drop trigger if exists validar_data_pagamento_vendas_mobile_trigger on public.vendas_mobile_pagamentos;
create trigger validar_data_pagamento_vendas_mobile_trigger
before insert or update of data_pagamento on public.vendas_mobile_pagamentos
for each row execute function public.validar_data_pagamento_vendas_mobile();

-- Recria a consolidação mensal a partir de recebimentos ocorridos até hoje.
-- Cada mês recebe sua própria entrada no Gestão, sem incluir datas futuras.
  ...
  for v_mes_reg in
    ...
    from (
      ...
      where v_base = 'recebidos'
        ...
        and p.data_pagamento <= (now() at time zone 'America/Sao_Paulo')::date
...
-- Aplica imediatamente a regra às receitas já consolidadas.
do $$
declare r record;
begin
  for r in
