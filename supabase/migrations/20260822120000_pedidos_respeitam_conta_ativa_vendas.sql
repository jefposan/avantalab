...
--
-- A rotina idempotente anterior (com três argumentos) continuava inserindo
-- sem conta_id. O gatilho de compatibilidade então escolhia a primeira conta
-- criada pelo usuário, o que desviava pedidos de contas adicionais/equipes.
  ...
  if coalesce(p_pedido ->> 'cliente_id', '') ~* v_uuid_pattern then
    ...
    if not exists (
      ...
      where cliente.id = v_cliente_id
        and cliente.conta_id = v_conta_id
    ) then
...
-- Repara somente registros que ficaram estruturalmente inconsistentes: o
-- cliente pertence a uma conta diferente da indicada pelo pedido/pagamento.
-- A marca evita recalcular o histórico financeiro enquanto a titularidade
-- operacional é corrigida.
select set_config('app.vendas_mobile_transferencia', '1', true);

update public.vendas_mobile_pedidos pedido
   set conta_id = cliente.conta_id,
       atualizado_em = now()
  from public.vendas_mobile_clientes cliente
