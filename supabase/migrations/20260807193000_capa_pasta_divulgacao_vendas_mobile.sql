alter table public.vendas_mobile_divulgacao_pastas
  add column if not exists capa_material_id uuid
  references public.vendas_mobile_divulgacao_materiais(id) on delete set null;

create index if not exists vendas_divulgacao_pastas_capa_idx
  on public.vendas_mobile_divulgacao_pastas (capa_material_id)
  where capa_material_id is not null;
...
before insert or update of capa_material_id, pasta_pai_id, empresa_id
on public.vendas_mobile_divulgacao_pastas
