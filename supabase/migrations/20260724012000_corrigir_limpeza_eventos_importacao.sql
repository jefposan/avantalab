...
as $$
declare v_empresa_id uuid; v_perfis_encontrados integer; v_locais integer := 0; v_clientes integer := 0;
begin
  if jsonb_typeof(p_linhas) <> 'array' or jsonb_array_length(p_linhas) = 0 then raise exception 'A planilha não possui linhas válidas para importar.'; end if;
  select count(*), (array_agg(id order by id))[1] into v_perfis_encontrados, v_empresa_id from public.empresas where upper(trim(nome)) = upper(trim(p_nome_perfil));
  ...
  if exists (select 1 from jsonb_to_recordset(p_linhas) as linha(status text, empresa text, subempresa text, valor numeric) where nullif(trim(linha.status), '') is null or nullif(trim(linha.empresa), '') is null or nullif(trim(linha.subempresa), '') is null or lower(trim(linha.status)) not in ('ativo', 'cancelado', 'inativo') or (linha.valor is not null and linha.valor <= 0)) then raise exception 'A importação contém status, nome ou valor inválido.'; end if;

  -- Eventos são removidos por cascata ao apagar lançamentos vinculados.
  ...
  with locais as (select distinct trim(linha.empresa) as nome from jsonb_to_recordset(p_linhas) as linha(status text, empresa text, subempresa text, valor numeric))
