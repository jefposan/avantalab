-- O Vendas Mobile gera uma única receita consolidada para cada mês do perfil
-- financeiro. A cada atualização, os valores mensais são recalculados e a
-- entrada recebe o dia corrente dentro do respectivo mês.
...
begin
  ...
  -- Remove a consolidação anterior e desconta seus valores dos meses em que
  -- estavam registrados antes de recriar o resumo mensal atualizado.
  for v_entrada in
  ...
  for v_mes_reg in
    ...
    from (
      ...
      where v_base = 'recebidos' and p.empresa_id = p_empresa_id
...
-- Recalcula toda empresa que já tenha resultado, pedido ou recebimento no
-- Vendas, garantindo a conversão do histórico para uma entrada por mês.
do $$
declare r record;
begin
  for r in
