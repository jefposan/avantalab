-- Importação transacional de clientes agrupados por local, usada para migrar
-- bases legadas sem deixar o módulo em estado parcial.

create or replace function public.recebimentos_importar_planilha(
  p_nome_perfil text,
  p_linhas jsonb
)
returns jsonb
...
begin
  ...
  -- Limpa exclusivamente os dados operacionais de Recebimentos do perfil.
  ...
  insert into public.recebimentos_empresas (
    empresa_id, nome, tipo_cadastro, endereco, cep, logradouro, bairro, cidade,
    estado, numero, complemento, responsavel, telefone, email, ativo
  )
  select v_empresa_id, locais.nome, 'local_agrupador', '', '', '', '', '', '', '', '', '', '', '', true
  from locais;
  get diagnostics v_locais = row_count;

  insert into public.recebimentos_subempresas (
    empresa_id, recebimento_empresa_id, nome, endereco, logradouro, numero,
    complemento, shopping_galeria, loja_sala, responsavel, cep, bairro, cidade,
    estado, valor_combinado, frequencia_recebimento, dias_semana, dia_mes,
    mes_inicio, dia_vencimento, ativo
  )
  select
    v_empresa_id,
    local.id,
    trim(linha.subempresa),
    '', '', '', '', '', '', '', '', '', '', '',
    linha.valor,
    'mensal', '{}', 30, null, 30,
    lower(trim(linha.status)) = 'ativo'
  from jsonb_to_recordset(p_linhas) as linha(status text, empresa text, subempresa text, valor numeric)
...
revoke all on function public.recebimentos_importar_planilha(text, jsonb) from public, authenticated;
grant execute on function public.recebimentos_importar_planilha(text, jsonb) to service_role;
