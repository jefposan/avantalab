...
- A verificação de conexão de um A1 já ativo passou a usar o evento protegido
  de revalidação já aceito pelo banco, distinguindo `connection_checked` somente
  nos metadados públicos. O ajuste preserva auditoria, isolamento da empresa e
  compatibilidade sem ampliar privilégios nem alterar a estrutura do cofre.
